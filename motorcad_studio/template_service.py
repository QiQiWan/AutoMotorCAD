...
class TemplateService:
    def __init__(self, inventory_path: Path, templates_dir: Path, registry: Registry):
    ...
    def _resolve_model_source(self, template_id: str, path: Path) -> dict[str, Any]:
        ...
        local_mot = self.root_dir / config["local_mot"] if config.get("local_mot") else None
        source_mtt = self.root_dir / config["source_mtt"] if config.get("source_mtt") else path
    ...
    def _load_templates(self) -> list[dict[str, Any]]:
        ...
        parameter_schema = self.registry.parameter_schema()
            ...
            templates.append(
                {
                    **item,
                    ...
                    "parameter_schema": {key: self.registry.parameter_schema(template_id)[key] for key in parameter_ids if key in parameter_schema},
