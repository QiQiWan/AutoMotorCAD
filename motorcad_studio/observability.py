...
class StructuredLogStore:
        ...
        *,
        ...
        with self._lock:
            ...
            record = {
                ...
                "trace_id": trace_id or task_id or request_id,
            ...
            if record["channel"] == "audit":
                self._write_json(self.audit_path, record)
            return record
    ...
    def cleanup_old_files(self) -> None:
        ...
        for path in self.root.iterdir() if self.root.exists() else []:
            if not path.is_file() or path.name in {".gitkeep"}:
                continue
            try:
                if path.stat().st_mtime < cutoff and (path.name.startswith("studio.") or path.name.startswith("audit.")):
                    path.unlink()
            except OSError:
                pass
    ...
    def export_bundle(self, target: Path, *, task_id: str | None = None, minutes: int | None = None, session_id: str | None = None) -> Path:
        ...
        with zipfile.ZipFile(target, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            ...
            central_names = {self.text_path.name, self.jsonl_path.name, self.audit_path.name}
            candidates = []
            for path in sorted(self.root.iterdir() if self.root.exists() else []):
                if not path.is_file():
                    continue
                if path.name in central_names or path.name.startswith(("studio.", "audit.")):
