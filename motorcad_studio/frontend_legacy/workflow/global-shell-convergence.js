/* V0.89-G5 Global Shell presentation authority.
...
(() => {
  ...
  const SHELL_STYLESHEET='/static/app.css';
  ...
  function ensureShellStylesheet(){
    return document.querySelector('link[href^="/static/app.css"]');
  }
  ...
  function refresh(){
    ...
    document.body?.classList.add('studio-current','studio-shell','studio-shell','studio-shell');
    document.body?.classList.toggle('results-stage-active',document.body?.dataset?.canonicalStage==='results');
    const result=audit();
