...
(() => {
  ...
  async function openIssueTarget(target,reason=''){
    ...
    ctl.returnToCheck={target:next,reason:String(reason||tr('从计算前检查进入此步骤处理阻断项。','Opened from the calculation check to resolve a blocker.'))};
  ...
  function renderReturnToCheckBanner(){
    ...
    const banner=document.createElement('div');banner.className='analysis-return-check-v090';banner.innerHTML=`<div><b>${tr('正在处理计算前检查项','Resolving a calculation-check item')}</b><small>${safe(context.reason||'')}</small></div><button type="button" data-return-analysis-check-v090>${tr('返回“检查并计算”','Return to Check and calculate')} →</button>`;
  ...
  function renderCheck(){
    ...
    const nativeDetail=fullRunning?(full?.message||tr('Motor-CAD 原生计算链路检查正在执行…','The native Motor-CAD calculation check is running…')):(full?.valid?tr('已为当前版本生成完整检查证据','Full check evidence was generated for the current revision'):full?.result?.motorcad?.message||full?.result?.message||tr('尚未执行；配置检查通过后可启动 Motor-CAD。','Not run yet. Motor-CAD can start after configuration checks pass.'));
    const flowHtml=[
      ...
      flowStep(3,tr('Motor-CAD 完整检查','Full Motor-CAD check'),full?.valid?tr('原生检查证据已固化','Native check evidence is recorded'):fullRunning?tr('后台检查正在运行','Background check is running'):needsNative?tr('可以开始原生检查','Ready to start the native check'):tr('等待配置检查通过','Waiting for configuration checks'),full?.valid?'done':fullRunning||needsNative?'active':'pending'),
    ...
    const primaryTitle=ready?tr('可以提交 Motor-CAD 计算','Ready to submit the Motor-CAD calculation'):fullRunning?tr('完整计算前检查正在执行','The full calculation check is running'):needsNative?tr('配置检查已通过，请执行 Motor-CAD 完整检查','Configuration checks passed. Run the full Motor-CAD check'):plan?tr(`还需处理 ${Math.max(1,hardIssues.length)} 项`,`Resolve ${Math.max(1,hardIssues.length)} item(s) before submission`):tr('先加载执行计划','Load the execution plan first');
    ...
    body.innerHTML=`<section class="analysis-editor-section-v076"><header><div><span class="eyebrow">5 · ${safe(stepLabels.check)}</span><h3>${tr('计算前检查与提交','Calculation check and submission')}</h3><p>${tr('所有检查、修正入口、Motor-CAD 原生检查和提交操作固定在此步骤；系统不会在检查过程中自动切换页面。','All checks, resolution actions, the native Motor-CAD check, and submission stay in this step. The system does not change pages automatically during checks.')}</p></div><div class="actions"><button id="analysisRefreshPlanV076" type="button" ${fullRunning?'disabled':''}>${tr('刷新执行计划','Refresh execution plan')}</button><button id="analysisFullCheckV076" type="button" ${(!plan||fullRunning||!configReady)?'disabled':''}>${fullRunning?tr('检查进行中…','Check in progress…'):tr('运行完整计算前检查','Run full calculation check')}</button></div></header>
      <ol class="analysis-check-flow-v090" aria-label="${tr('计算前检查流程','Calculation-check flow')}">${flowHtml}</ol>
      ...
      <div id="analysisCheckStatusV076" class="analysis-inline-status-v076 ${fullRunning?'running':''}">${fullRunning?safe(full.message||tr('完整计算前检查正在运行…','The full calculation check is running…')):(full?.valid?tr('完整计算前检查已通过，可以提交 Motor-CAD 计算。','The full calculation check passed. The Motor-CAD calculation can be submitted.'):full?.result?.message?safe(full.result.message):'')}</div>
    </section>`;
  ...
  async function runFullCheck(){
    ...
    const op=operationProgress({id:`analysis-full-precheck-${ctl.active.id}`,label:'完整计算前检查',stage:'创建后台检查任务',detail:'将立即返回任务标识，Motor-CAD 检查在后台执行',percent:3,button:firstButton,failDelay:6000,timeoutMs:270000,timeoutDetail:'计算前检查超过 4 分 30 秒，界面已恢复；请查看 Motor-CAD 进程与服务日志。'});
    ...
    try{
      ...
      while(['QUEUED','RUNNING'].includes(String(current.status||'').toUpperCase())){
        if(Date.now()>deadline)throw new Error('计算前检查等待超时；界面已恢复，请查看 Motor-CAD 运行日志后重试。');
        attempt+=1;
