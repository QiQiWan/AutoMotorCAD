...
(function(){
  ...
  function renderMatrix(matrix){
    ...
    const semText=`精确语义绑定 ${sem.qualified_count||0}/${sem.golden_template_count||3} 个 Golden 模板已固化；原生模型回读 ${readbackQualified}/${rows.length} 个 Profile 为 QUALIFIED；故障树/修复编排 ${repairClean}/${rows.length} 个 Profile 为 CLEAN。`;
    q('#nativeParitySummaryClosure').textContent=(matrix?.complete?'当前资格 Profile 已全部通过语义绑定、原生模型回读、故障树/修复编排和求解证据门禁。':`已通过 ${matrix?.qualified_profiles||0}/${matrix?.total_profiles||rows.length} 个 Profile；原生模型回读存在漂移/缺口，或 RepairPlan 非 CLEAN 时不会获得正式资格。`)+` ${semText}`;
