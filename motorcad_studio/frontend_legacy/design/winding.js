...
(() => {
  ...
  const {safe,number,clamp,fmt,tr,unitLabel,parameterRecord,selectAttribute,polar,viewData,authorityStrip,phaseColors}=U;
  const patternLabel=value=>({Concentrated:tr('集中绕组','Concentrated winding'),Distributed:tr('分布绕组','Distributed winding')})[String(value||'')]||String(value||tr('模板继承','Inherited from template'));
  ...
  function windingView(ctx){
    ...
    return`<div class="motorcad-view-v031 winding-view-v031"><div class="visual-heading-v031"><div><span class="eyebrow">绕组排布</span><h3>绕组排布、槽号与并联支路</h3><p>槽号、相序和并联支路会随当前设计参数同步更新。</p></div><div class="visual-facts-v031"><span>${phases} 相</span><span>${turns} 匝 / 线圈</span><span>${paths} 并联支路</span></div></div>
      <div class="winding-layout-v031"><div class="winding-canvas-v031"><svg viewBox="0 0 600 550" role="img" aria-label="带槽号和并联支路的绕组相槽关系预览"><circle cx="${cx}" cy="${cy}" r="210" class="winding-stator-v031"/><circle cx="${cx}" cy="${cy}" r="112" class="winding-rotor-v031"/><g>${coils}</g><g>${marks}</g><g>${slotNumbers}</g><text x="${cx}" y="248" text-anchor="middle" class="winding-title-v031">${slots} 槽 / ${poleCount} 极</text><text x="${cx}" y="274" text-anchor="middle" class="winding-subtitle-v031">${phases} 相 · ${paths} 支路 · 预估节距 ${throwSlots} 槽</text><text x="${cx}" y="298" text-anchor="middle" class="winding-subtitle-v031">${safe(patternLabel(w.pattern_class))} · ${safe(w.slot_arrangement||'模板继承')}</text></svg></div>
      <aside class="winding-summary-v031"><div class="winding-health-v031 ${valid?'pass':'blocked'}"><span>${valid?'✓':'!'}</span><div><small>每相每支路槽数</small><b>${q!==null&&q!==undefined?fmt(q,5):'等待模型检查'}</b><p>${valid?'槽、相与支路关系可整数分配':'当前关系无法整数分配'}</p></div></div><div class="winding-evidence-v035 ${hasNative?'native':'preview'}"><b>${hasNative?'当前计算采用的线圈槽对':'设计版本即时预览'}</b><span>${hasNative?`${nativeCoils.length} 个线圈槽对已用于当前视图`:'当前为设计预览；完成 Motor-CAD 检查后显示实际线圈与支路定义'}</span></div><div class="winding-metrics-v031"><div><span>绕组层数</span><b>${fmt(w.layers,0)}</b></div><div><span>槽满率输入</span><b>${fmt(values.slot_fill_factor??w.slot_fill_factor)}</b></div><div><span>线圈节距</span><b>${hasNative?'按计算槽对':`${throwSlots} <em>视图估计</em>`}</b></div><div><span>并联支路</span><b>${paths}</b></div></div><div class="phase-legend-v031">${Array.from({length:phases},(_,i)=>`<span style="--phase:${phaseColors[i%phaseColors.length]}">相 ${i+1}</span>`).join('')}${paths>1?Array.from({length:paths},(_,i)=>`<span class="branch-legend-v066">P${i+1}</span>`).join(''):''}</div><div class="winding-slot-table-v031"><table><thead><tr><th>槽号</th><th>相</th><th>方向</th><th>支路</th></tr></thead><tbody>${rows}</tbody></table>${slots>36?`<small>显示前 36 / ${slots} 槽；模型仍标注全部或隔槽编号</small>`:''}</div></aside></div>${authorityStrip(hasNative?'当前计算采用的 Motor-CAD 绕组槽对':'当前显示设计参数生成的绕组预览；完成 Motor-CAD 检查后切换为实际绕组定义')}
    </div>`;
  }
  ...
  function slotView(ctx){
    ...
    const dividerW=clamp(slotWidthPx*.055,8,18),markerPlan=D?.conductorMarkerPlan?.(turns,{maxMarkers:96})||{marker_count:Math.min(turns,96),turns_per_marker:turns/Math.min(turns,96),exact:turns<=96},target=markerPlan.marker_count,radius=clamp(6.2-Math.max(0,target-42)*.035,3.2,6.2),gapPx=2.4;
