/* V0.71 Parameter Inspector shared by read-only Design Viewer and Draft editor.
 * PM topology applicability and per-view parameter ownership come from the PM Motor Object. */
(() => {
  const U=window.MCSDesignRenderUtils;if(!U)throw new Error('MCSDesignRenderUtils must load before parameter inspector');
  const {safe,fmt,tr,unitLabel,parameterRecord}=U;
  ...
  function objectFor(data,values=null){return window.MCSMotorObject?.resolve?.(data,values||data?.effective_parameters||{},data?.materials)||window.MCSPMMotorObject?.resolve?.(data,values||data?.effective_parameters||{},data?.materials)||null}
  function applicableIds(data){const object=objectFor(data);return new Set(object?.applicable_parameter_ids||[])}
  function readOnlyPanel(view,data,{selectedParameter=null}={}){
    const spec=(data.design_views||[]).find(row=>row.id===view)||{},values=data.effective_parameters||{},object=objectFor(data,values),objectIds=object?.visualization?.view_parameter_ids?.[view]||[],ids=objectIds.length?objectIds:(spec.parameter_ids||[]);
    ...
    if(view==='materials')return`<aside class="design-context-panel-v031"><span class="eyebrow">材料配置</span><h4>当前部件材料</h4><p>材料随电机版本保存；冷却介质在分析配置中设置。</p><div class="context-rule-v031"><b>如何更换材料</b><span>进入材料配置后，在目标部件行点击“选择材料”；材料库会自动带入当前部件。</span></div><button type="button" class="primary edit-view-v031" data-edit-view-v031="materials">进入材料配置</button></aside>`;
  ...
  function editorGroupButtons({data,group,changed=new Set(),filter=''}){
    const needle=String(filter||'').trim().toLowerCase(),allowed=applicableIds(data);
    return(data?.groups||[]).map(row=>{
      const rows=(data?.parameters||[]).filter(p=>p.category===row.id&&(!allowed.size||allowed.has(p.id)));
  ...
  function editorRegionButtons({data}){
    const allowed=applicableIds(data);
    return Object.entries(data?.regions||{}).map(([id,row])=>`<button type="button" data-workbench-region="${safe(id)}"><span>${safe(row.label||id)}</span><small>${(row.parameter_ids||[]).filter(pid=>parameterRecord(data,pid)&&(!allowed.size||allowed.has(pid))).length} 参数</small></button>`).join('');
  }

  function editorParameterRows({data,group,values={},changed=new Set(),selected=null}){
    const allowed=applicableIds(data),rows=(data?.parameters||[]).filter(row=>row.category===group&&(!allowed.size||allowed.has(row.id)));
    return rows.map(row=>{
      const isChanged=changed.has(row.id),isSelected=selected===row.id;
      return`<article class="workbench-param-row-v024 ${isChanged?'changed':''} ${isSelected?'selected':''}" data-workbench-param-row="${safe(row.id)}"><button type="button" class="workbench-param-focus-v024" data-workbench-select="${safe(row.id)}"><span><b>${safe(row.label)}</b><small>${safe(row.description||tr('修改后立即更新模型预览','The model preview updates immediately'))}</small></span><em>${row.explicit?tr('已单独设置','Explicit value'):tr('采用模板值','Using template value')}</em></button><div class="workbench-param-control-v024"><input aria-label="${safe(row.label)}" data-workbench-input="${safe(row.id)}" type="number" step="${row.type==='integer'?'1':'any'}" ${row.minimum!==null&&row.minimum!==undefined?`min="${safe(row.minimum)}"`:''} ${row.maximum!==null&&row.maximum!==undefined?`max="${safe(row.maximum)}"`:''} value="${safe(display(values[row.id]))}"><span>${safe(unitLabel(row.unit||''))}</span></div><div class="workbench-param-lineage-v024"><span>${tr('模板值','Template')} ${safe(display(row.template_default))}</span><span>${tr('上一可行值','Last feasible')} ${safe(display(row.previous_feasible_value))}</span></div><div class="workbench-param-actions-v024"><button type="button" data-workbench-restore="previous" data-param-id="${safe(row.id)}" ${row.previous_feasible_value===undefined?'disabled':''}>${tr('恢复上一可行值','Restore last feasible')}</button><button type="button" data-workbench-restore="template" data-param-id="${safe(row.id)}" ${row.template_default===undefined?'disabled':''}>${tr('恢复模板值','Restore template')}</button></div></article>`;
    }).join('')||`<div class="workspace-empty compact">${tr('当前分组没有可编辑设计参数。','This group has no editable design parameters.')}</div>`;
  }
