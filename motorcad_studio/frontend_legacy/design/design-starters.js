...
(() => {
  ...
  const card=s=>`<article class="golden-starter-card-v087" data-starter-card="${esc(s.id)}">
    ...
    <div class="starter-flow-v087"><span>${tr('预制设计','Starter design')}</span><i>→</i><span>${revisionLabel(1)}</span><i>→</i><span>${tr('设计验证','Design validation')}</span><i>→</i><span>${tr('结果决策','Result decision')}</span></div>
  ...
  function inputRow(spec){const current=spec.default_value??'';const range=(spec.recommended_min!=null&&spec.recommended_max!=null)?`建议 ${spec.recommended_min}–${spec.recommended_max} ${spec.unit||''}`:'按模板默认值';return `<label class="starter-input-v087"><span>${esc(spec.label)}${spec.required?'<b class="required-mark-v087">必填</b>':''}</span><div class="input-with-unit-v087"><input type="number" data-starter-input="${esc(spec.parameter_id)}" data-starter-default="${esc(current)}" value="${esc(current)}" ${spec.step!=null?`step="${esc(spec.step)}"`:''} ${spec.hard_min!=null?`min="${esc(spec.hard_min)}"`:''} ${spec.hard_max!=null?`max="${esc(spec.hard_max)}"`:''}><em>${esc(spec.unit||'')}</em></div><small>${esc(range)}</small></label>`}
  function openCreate(id){
    ...
    host.innerHTML=`<div class="section-head"><div><span class="eyebrow">${esc(starter.short_label)} · ${tr('引导创建','GUIDED CREATE')}</span><h2>创建 ${esc(starter.label)}</h2><p>首次只确认设计名称和少量关键参数；未填写项继续使用受控模板默认值。</p></div><button type="button" data-starter-close>关闭</button></div>
      ...
      <div class="actions"><button type="button" data-starter-close>取消</button><button id="goldenStarterConfirmV087" type="button" class="primary" ${pid?'':'disabled'}>创建预制设计 + ${revisionLabel(1)}</button></div><div id="goldenStarterCreateStatusV087" class="hint"></div>`;
  ...
  function openDetail(id){
    ...
    host.innerHTML=`<div class="section-head"><div><span class="eyebrow">${tr('工程范围','ENGINEERING SCOPE')}</span><h2>${esc(s.label)}</h2><p>${esc(s.topology_label)} · ${esc(s.application)}</p></div><button type="button" data-starter-close>关闭</button></div><div class="starter-detail-grid-v087"><div><span>设计参数组</span><b>${(s.design_groups||[]).map(esc).join(' / ')}</b></div><div><span>标准分析包</span><b>${(s.standard_analysis_steps||[]).map(x=>esc(x.short_label||x.label)).join(' / ')}</b></div><div><span>${tr('结果指标卡','Result scorecard')}</span><b>${(s.scorecard_metrics||[]).map(x=>esc(x.label||x.metric_id)).join(' / ')}</b></div><div><span>优化变量</span><b>${(s.optimization_parameter_specs||[]).map(x=>esc(x.label||x.parameter_id)).join(' / ')}</b></div></div>${Object.keys(s.mapping_readiness?.deferred_parameters||{}).length?`<div class="callout info"><b>暂缓开放参数</b><br>${Object.entries(s.mapping_readiness.deferred_parameters).map(([id,row])=>`${esc(id)}：${esc(row.reason||row.status)}`).join('<br>')}</div>`:''}<div class="callout warning"><b>当前资格状态</b><br>${esc(qualificationLabel(s.qualification))}。Studio 不会把“模板存在”误标记为 Windows Motor-CAD 生产验证通过。</div><div class="actions"><button class="primary" type="button" data-starter-use="${esc(s.id)}">使用此预制设计</button></div>`;
  ...
  async function createStarter(starter){
    ...
    const btn=q('#goldenStarterConfirmV087'),status=q('#goldenStarterCreateStatusV087');if(btn)btn.disabled=true;if(status)status.textContent=`正在创建受控${revisionLabel(1)}…`;
    const inputs={};qa('[data-starter-input]').forEach(el=>{if(el.value!==''&&Number(el.value)!==Number(el.dataset.starterDefault))inputs[el.dataset.starterInput]=Number(el.value)});
    ...
    try{
      ...
      if(status)status.textContent='创建完成，正在进入电机设计工作台。';window.toast?.(`已创建 ${name} · ${revisionLabel(1)}`,'SUCCESS',6000);
  ...
  document.addEventListener('DOMContentLoaded',load,{once:true});window.addEventListener('mcs:engineering-context-changed',()=>bind());document.addEventListener('mcs-language-change',()=>state.payload&&render());
