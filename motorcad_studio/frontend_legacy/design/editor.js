...
(() => {
    ...
    canvas?.addEventListener('click', async event => {
      ...
      const material = event.target.closest('[data-workbench-material-component]'); if (material) {
        ...
        try { await window.MCSMaterialLibrary.pick({kind: 'solid', materialType: type, componentLabel, title: window.MCS_I18N?.t?.(`选择${componentLabel}材料`,`Choose material for ${componentLabel}`)||`选择${componentLabel}材料`, onSelect: row => {
          ...
          const before = objectSnapshot(wb.materials), selectedDatabase = row.source_database_path || wb.materials.material_database_path || null;
          if (row.source_database_path && wb.materials.material_database_path && wb.materials.material_database_path !== row.source_database_path) toast(window.MCS_I18N?.t?.('当前部件来自另一材料数据库；本草稿的 Motor-CAD 材料数据库已切换到新来源。请确认其他自定义材料也存在于该数据库。','This component comes from another material database. The draft now uses the new Motor-CAD database; confirm that other custom materials also exist there.')||'材料数据库已切换。','WARNING',7000);
          wb.materials = {...wb.materials, material_database_path: selectedDatabase, component_materials: components, material_provenance: provenance}; wb.materialDirty = objectSnapshot(wb.materials) !== objectSnapshot(wb.baseMaterials);
  ...
  async function open(routeCtx = null) {
    ...
    try {
      ...
      const runtimeSnapshot = window.MotorCADStudio?.legacyRuntime?.snapshot?.() || null;
      window.MCSPageRuntime?.report?.('DESIGN_EDITOR_LOAD_FAILED', 'ERROR', '设计工作台加载失败', {
        ...
        requested_route: routeCtx?.route?.path || routeCtx?.path || location.pathname,
        legacy_owner: runtimeSnapshot?.currentOwner || null,
      });
      // A failed open must leave the editor in a clean, retryable state.
      // Otherwise a partially initialized draft/verification controller can keep
      // later attempts in a zombie edit mode even after the original exception is gone.
      wb.shellAbort?.abort(); wb.shellAbort = null; verification?.dispose?.();
      wb.revisionId = null; wb.data = null; wb.values = {}; wb.baseValues = {};
      wb.changed = new Set(); wb.materials = {}; wb.baseMaterials = {}; wb.materialDirty = false;
      wb.explicitIds = new Set(); wb.selected = null; wb.saveBusy = false; wb.leavePrepared = false;
      document.body.classList.remove('design-editing-v062');
      window.MCSDesignStore?.patch?.({mode: 'read', dirtyCount: 0, draftStatus: 'error'}, {source: 'editor-load-failed'});
      if (canvas) canvas.innerHTML = `<div class="workspace-empty"><b>设计工作台加载失败</b><p>${safe(error.message || error)}</p><button type="button" data-retry-design-editor>重新加载设计工作台</button></div>`;
      const retry = canvas?.querySelector?.('[data-retry-design-editor]');
      if (retry) retry.addEventListener('click', () => { void open(); }, {once: true});
      toast(error.message || String(error), 'ERROR', 8000); return false;
    }
  }
