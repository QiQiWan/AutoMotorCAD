...
(() => {
  ...
  const stateV061={status:null,records:[],selected:null,query:'',kind:'',materialType:'',searchTimer:null,picker:null,resize:null,autoScanAttempted:false,scanBusy:false};
  const propertyMeta={
  ...
  function renderList(){
    ...
    if(stateV061.records.length){
      box.innerHTML=stateV061.records.map(recordItem).join('');
    }else{
      const total=Number(stateV061.status?.records||0),hasDb=(stateV061.status?.databases||[]).length>0;
      const title=total? '当前筛选条件没有匹配材料。' : '材料库当前为空。';
      const detail=total
        ? '可以清除筛选条件，或重新扫描 Motor-CAD 材料数据库。'
        : hasDb
          ? '已登记材料数据库但没有可用记录，请重新扫描或检查数据库内容。'
          : '尚未载入 Motor-CAD 的 Solids.mdb / Fluids.mdb。Studio 可以自动扫描本机默认数据库，也可以在材料数据库管理中按路径导入。';
      box.innerHTML=`<div class="material-list-empty-v061 material-list-empty-action-v0916"><b>${safe(title)}</b><span>${safe(detail)}</span><div><button type="button" class="primary" data-material-empty-scan-v0916>${stateV061.scanBusy?'正在扫描…':'扫描本机材料库'}</button><button type="button" data-material-open-manager-v0916>材料数据库管理</button></div></div>`;
      q('[data-material-empty-scan-v0916]',box)?.addEventListener('click',()=>scan());
      q('[data-material-open-manager-v0916]',box)?.addEventListener('click',()=>openManager());
    }
  ...
  async function scan(options={}){
    if(stateV061.scanBusy)return null;
    const silent=Boolean(options.silent),refresh=options.refresh!==false;
    const buttons=qa('[data-material-scan-v061],[data-material-picker-scan-v0916],[data-material-empty-scan-v0916]');
    stateV061.scanBusy=true;buttons.forEach(button=>{button.disabled=true;button.dataset.originalLabel=button.textContent;button.textContent='正在扫描…'});renderList();
    try{
      const result=await api('/api/material-library/scan',{method:'POST',body:'{}'});
      const count=(result.imported||[]).reduce((sum,row)=>sum+Number(row.material_count||0),0);
      if(!silent)toast(result.imported?.length?`扫描完成：载入 ${result.imported.length} 个数据库、${count} 条材料记录。`:'没有自动发现可读取的 Motor-CAD .mdb。请打开“材料数据库管理”，核对默认数据库位置或按路径导入。',result.imported?.length?'SUCCESS':'WARNING',7000);
      if(refresh)await refreshAll(null,{autoScan:false});
      return result;
    }catch(error){if(!silent)toast(`扫描失败：${error.message}`,'ERROR',7000);return null}
    finally{stateV061.scanBusy=false;buttons.forEach(button=>{button.disabled=false;button.textContent=button.dataset.originalLabel||'扫描本机材料库'});renderList()}
  }
  async function openManager(){stateV061.picker=null;stateV061.query='';stateV061.kind='';stateV061.materialType='';renderShellBody();await refreshAll(null,{autoScan:false})}
  ...
  function bindShell(node){
    q('[data-material-scan-v061]',node)?.addEventListener('click',()=>scan());q('[data-material-picker-scan-v0916]',node)?.addEventListener('click',()=>scan());q('[data-material-open-manager-v0916]',node)?.addEventListener('click',()=>openManager());q('[data-material-import-v061]',node)?.addEventListener('click',importPath);q('[data-material-new-v061]',node)?.addEventListener('click',newRecord);q('[data-material-export-solid-v061]',node)?.addEventListener('click',()=>exportManaged('solid'));q('[data-material-export-fluid-v061]',node)?.addEventListener('click',()=>exportManaged('fluid'));
  ...
  function renderShellBody(){
    ...
    const banner=picker?`<div class="material-picker-banner-v062 material-picker-banner-v0916"><div><b>正在为：${safe(picker.componentLabel||'当前部件')} 选择材料</b><span>单击查看属性/曲线；双击材料可直接赋值。若列表为空，可扫描本机 Motor-CAD 数据库或进入材料数据库管理。</span></div><div class="material-picker-actions-v0916"><button type="button" class="primary" data-material-picker-scan-v0916>${stateV061.scanBusy?'正在扫描…':'扫描本机材料库'}</button><button type="button" data-material-open-manager-v0916>材料数据库管理</button></div></div>`:'';
  ...
  async function refreshAll(preferredId=null,options={}){try{
    stateV061.status=await api('/api/material-library/status');
    const shouldAutoScan=options.autoScan!==false&&Boolean(stateV061.picker)&&Number(stateV061.status?.records||0)===0&&!stateV061.autoScanAttempted;
    if(shouldAutoScan){stateV061.autoScanAttempted=true;await scan({silent:true,refresh:false});stateV061.status=await api('/api/material-library/status')}
    await refreshRecords(preferredId);renderShellBody();if(preferredId)await selectRecord(preferredId)
  }catch(error){const body=q('#materialLibraryBodyV061');if(body)body.innerHTML=`<div class="material-detail-empty-v061"><b>材料模块无法加载</b><span>${safe(error.message)}</span></div>`;toast(`材料模块加载失败：${error.message}`,'ERROR',7000)}}
  async function open(options={}){stateV061.picker=options?.picker?options:null;if(stateV061.picker){stateV061.kind=options.kind||'solid';stateV061.materialType=options.materialType||'';stateV061.query='';stateV061.selected=null}shell();await refreshAll(null,{autoScan:true})}
  ...
  window.MCSMaterialLibrary={open,pick,close,refresh:refreshAll,scan,openManager,state:stateV061};
})();
