...
(() => {
  ...
  const network = {count: 0, failed: 0, timer: null, op: null, lastClickAt: 0, generation: 0, foregroundGetAvailable: false};
  ...
  function trackRequest(url, options = {}) {
    ...
    // Background GETs (polling, bootstrap fan-out, read-model refreshes) are not
    // user operations.  At most one GET after a click may claim the generic card;
    // complex flows use an explicit MCSOperationProgress operation instead.
    const foregroundGet = method === 'GET' && (
      options?.__mcsForegroundProgress === true || (recentClick && network.foregroundGetAvailable)
    );
    if (method === 'GET' && !foregroundGet) return () => {};
    if (foregroundGet && options?.__mcsForegroundProgress !== true) network.foregroundGetAvailable = false;
    const label = method === 'GET'
      ? `${tr('刷新', 'Refresh ')}${domain}`
      : `${tr('保存', 'Save ')}${domain}`;
  ...
  document.addEventListener('click', event => {
    ...
    network.foregroundGetAvailable = true;
  ...
  document.body?.classList.add('results-stage-active3', 'studio-shell');
})();
