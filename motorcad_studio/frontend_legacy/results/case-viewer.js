...
(() => {
// Result viewer --------------------------------------------------------------------
const viewerProgress=options=>window.MCSOperationProgress?.start?.(options)||{update(){return this},done(){return this},fail(){return this},close(){return this}};
...
function viewerModuleNeedsResult(key,row){const id=String(row?.result_id||row?.id||'').toLowerCase(),type=String(row?.result_type||row?.type||''),declared=Array.isArray(row?.viewer_modules)?row.viewer_modules.map(String):[];if(!row?.externalized&&!row?.data_ref)return false;if(declared.length)return declared.includes(key);if(key==='waveforms')return type==='series'&&!id.includes('harmonic');if(key==='harmonics')return type==='spectrum'||(type==='series'&&id.includes('harmonic'));if(key==='fea')return ['map','field','vector_field'].includes(type)&&(/fea|field|flux|stress|b_|vector/.test(id)||type==='field'||type==='vector_field');if(key==='thermal'||key==='temperatures'||key==='thermal_schematic')return ['series','map','field','vector_field','table'].includes(type)&&/temp|heat|thermal|coolant/.test(id);if(key==='lab')return type==='map'&&/map|lab|efficiency|loss|torque_speed/.test(id);if(key==='mechanical'||key==='stress'||key==='nvh')return ['series','map','field','vector_field','table'].includes(type)&&/stress|force|nvh|modal|campbell|displacement/.test(id);if(key==='graphs'||key==='output_data')return ['series','spectrum','map','field','vector_field','table'].includes(type);return false}
async function ensureViewerModuleData(key){const viewer=state.viewer,bundleId=viewer?.case?.result_bundle_id;if(!viewer||!bundleId||viewer.heavy_data_hydrated||!window.MCSResultDataGateway?.get)return;const inventory=viewer.result_data_inventory||state.resultBundleAggregate?.result_inventory?.items||[],rows=inventory.filter(row=>viewerModuleNeedsResult(key,row));if(!rows.length)return;viewer._heavyLoaded=viewer._heavyLoaded||{};const pending=rows.filter(row=>!viewer._heavyLoaded[row.result_id||row.id]);if(!pending.length)return;const canvas=$('#viewerCanvas'),op=viewerProgress({id:`viewer-heavy-${bundleId}-${key}`,label:'加载结果模块',stage:'读取大结果数据',detail:`${key} · 0/${pending.length}`,percent:8});if(canvas)canvas.innerHTML='<div class="viewer-loading-v058"><span class="spinner-dot"></span><b>正在按需读取大结果数据…</b></div>';let completed=0;try{await Promise.all(pending.map(async row=>{const id=row.result_id||row.id,payload=await window.MCSResultDataGateway.get(bundleId,id),bucket=viewerResultBucket(row.result_type||row.type);if(bucket){viewer.results=viewer.results||{};viewer.results[bucket]=viewer.results[bucket]||{};viewer.results[bucket][id]=payload?.data}viewer._heavyLoaded[id]=true;completed+=1;op.update({percent:8+Math.round(86*completed/pending.length),stage:'解析结果块',detail:`${key} · ${completed}/${pending.length}`})}));op.done(`${key} 结果数据已加载`)}catch(error){op.fail(error.message);throw error}}
...
function renderResultTable(table,title){
  ...
  const first=rows[0],headers=Array.isArray(first)?first.map((_,i)=>`C${i+1}`):Object.keys(first||{});if(!headers.length)return'';
  const body=rows.slice(0,100).map(row=>`<tr>${headers.map(h=>`<td>${esc(Array.isArray(row)?row[Number(h.slice(1))-1]:row?.[h]??'—')}</td>`).join('')}</tr>`).join('');
  return `<details class="viewer-data-group" open><summary>${esc(title)} · ${rows.length} ${uiText('行','rows')}</summary><div class="table-wrap"><table><thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table></div>${rows.length>100?`<small>${uiText('页面仅预览前100行，完整数据请使用原始结果导出。','Only the first 100 rows are previewed; use raw result export for the complete table.')}</small>`:''}</details>`;
}
function renderViewerOutputData(v){
  const r=v.results||{},scalars=r.scalars||{},series=r.series||{},maps=r.maps||{},fields=r.fields||{},vectors=r.vectors||{},tables=r.tables||{};
  const scalarRows=Object.keys(scalars).map(k=>`<tr><td>${esc(scalarLabel(k))}</td><td>${esc(scalars[k])}</td><td>${esc(scalarUnit(k)||'—')}</td><td><code>${esc(k)}</code></td></tr>`).join('');
  const inventory=[['曲线',series],['二维图',maps],['场',fields],['矢量场',vectors],['表格',tables]].map(([name,obj])=>`<div><span>${name}</span><b>${Object.keys(obj||{}).length}</b></div>`).join('');
  const tableHtml=Object.entries(tables).map(([k,t])=>renderResultTable(t,scalarLabel(k))).join('');
  return `<div class="viewer-section-title"><h3>${uiText('结构化输出数据','Structured output data')}</h3><p>${uiText('按当前 ResultBundle 中已实际提取的数据展示，不使用占位值。','Shows only data actually extracted into the current ResultBundle; no placeholder values are injected.')}</p></div><div class="result-data-inventory-v089g3"><div><span>${uiText('标量','Scalars')}</span><b>${Object.keys(scalars).length}</b></div>${inventory}</div>${scalarRows?`<div class="table-wrap"><table><thead><tr><th>${uiText('结果','Result')}</th><th>${uiText('值','Value')}</th><th>${uiText('单位','Unit')}</th><th>ID</th></tr></thead><tbody>${scalarRows}</tbody></table></div>`:genericEmpty(uiText('没有标量输出','No scalar outputs'),uiText('当前算例没有提取标量结果。','No scalar result was extracted for this case.'))}${tableHtml}`;
}
function renderViewerGraphs(v){
  const r=v.results||{},series=Object.entries(r.series||{}),maps=Object.entries(r.maps||{}),fields=Object.entries(r.fields||{}),vectors=Object.entries(r.vectors||{});
  const html=series.map(([k,x])=>renderLineSvg(x,scalarLabel(k))).join('')+maps.map(([k,x])=>renderHeatMap(x,scalarLabel(k))).join('')+fields.map(([k,x])=>renderMeshField(x,scalarLabel(k))).join('')+vectors.map(([k,x])=>renderVectorField(x,scalarLabel(k))).join('');
  return html||genericEmpty(uiText('当前算例没有可绘制图形','No graph data for this case'),uiText('需要的曲线或场结果尚未提取，可返回分析配置补充结果请求。','Required curves or fields were not extracted; add them in analysis configuration if needed.'));
}
...
function renderMechanicalFamily(v,mode='mechanical'){
  ...
  let html='';Object.entries(r.series||{}).filter(([k])=>match.test(k)).forEach(([k,x])=>html+=renderLineSvg(x,scalarLabel(k)));Object.entries(r.maps||{}).filter(([k])=>match.test(k)).forEach(([k,x])=>html+=renderHeatMap(x,scalarLabel(k)));Object.entries(r.fields||{}).filter(([k])=>match.test(k)).forEach(([k,x])=>html+=renderMeshField(x,scalarLabel(k)));Object.entries(r.vectors||{}).filter(([k])=>match.test(k)).forEach(([k,x])=>html+=renderVectorField(x,scalarLabel(k)));Object.entries(r.tables||{}).filter(([k])=>match.test(k)).forEach(([k,x])=>html+=renderResultTable(x,scalarLabel(k)));
  return html||genericEmpty(uiText('当前算例没有对应机械结果','No matching mechanical result for this case'),uiText('该模块已实现数据展示，但当前 ResultBundle 没有提取对应输出。','The view is implemented, but the current ResultBundle contains no matching output.'));
}

function renderViewerModule(key){
  if(!state.viewer)return;state.activeViewerModule=key;renderViewerModuleNav(key);
  const v=state.viewer,canvas=$('#viewerCanvas'),inspector=$('#viewerInspector');if(!canvas)return;const series=v.results?.series||{},maps=v.results?.maps||{};let html='';
  if(key==='overview')html=renderViewerOverview(v);
  else if(key==='performance')html=renderViewerPerformance(v);
  else if(key==='losses')html=renderViewerLosses(v);
  else if(key==='output_data')html=renderViewerOutputData(v);
  else if(key==='graphs')html=renderViewerGraphs(v);
  else if(key==='inputs')html=renderInputTables(v);
  else if(key==='waveforms'){const rows=Object.entries(series).filter(([k])=>!k.toLowerCase().includes('harmonic'));html=rows.map(([k,x])=>renderLineSvg(x,scalarLabel(k))).join('')||genericEmpty(uiText('当前算例没有已提取波形','No extracted waveform for this case'),uiText('需要的电磁波形尚未进入 ResultBundle。','The requested electromagnetic waveform is not present in this ResultBundle.'))}
  else if(key==='harmonics'){const rows=Object.entries(series).filter(([k,x])=>k.toLowerCase().includes('harmonic')||x?.kind==='spectrum');html=rows.map(([k,x])=>renderLineSvg(x,scalarLabel(k))).join('')||genericEmpty(uiText('当前算例没有谐波频谱','No harmonic spectrum for this case'),uiText('请在分析配置中请求相应谐波输出。','Request the corresponding harmonic output in analysis configuration.'))}
  else if(key==='fea'){const rows=Object.entries(maps).filter(([k])=>/fea|field|flux|b_/i.test(k)),fields=Object.entries(v.results?.fields||{}),vectors=Object.entries(v.results?.vectors||{});html=rows.map(([k,m])=>renderHeatMap(m,scalarLabel(k))).join('')+fields.map(([k,f])=>renderMeshField(f,scalarLabel(k))).join('')+vectors.map(([k,f])=>renderVectorField(f,scalarLabel(k))).join('');if(!html)html=genericEmpty(uiText('结构化 FEA 视图正在读取原生场数据','Structured FEA view is loading native field data'),uiText('下方原生有限元场查看器会直接读取当前计算记录的 FEA 证据。','The native FEA viewer below reads field evidence directly from the current calculation.'))}
  else if(key==='thermal')html=renderThermalFamily(v,'thermal');
  else if(key==='thermal_schematic')html=renderThermalFamily(v,'thermal_schematic');
  else if(key==='temperatures')html=renderThermalFamily(v,'temperatures');
  else if(key==='lab'){const rows=Object.entries(maps).filter(([k])=>/map|lab|efficiency|loss|torque_speed/i.test(k));html=rows.map(([k,m])=>renderHeatMap(m,scalarLabel(k))).join('')||genericEmpty(uiText('当前算例没有 Lab 二维图谱','No Lab map for this case'),uiText('效率图、损耗图和转矩-转速包络需要对应 Lab 结果输出。','Efficiency, loss and torque-speed maps require the corresponding Lab outputs.'))}
  else if(key==='mechanical')html=renderMechanicalFamily(v,'mechanical');
  else if(key==='stress')html=renderMechanicalFamily(v,'stress');
  else if(key==='nvh')html=renderMechanicalFamily(v,'nvh');
  else if(key==='artifacts')html=`<div class="artifact-browser">${(v.artifacts||[]).map(a=>`<a class="artifact-card" href="/api/artifacts/${a.id}" target="_blank"><b>${esc(a.name||a.kind)}</b><span>${esc(a.kind||'')} · ${a.size_bytes?Math.round(a.size_bytes/1024)+' KB':''}</span></a>`).join('')||genericEmpty(uiText('没有原生结果文件','No native artifacts'),uiText('当前计算记录没有已归档 Artifact。','No artifact was archived for this case.'))}</div>`;
  else html=genericEmpty(uiText('当前视图暂无可展示数据','No data available in this view'),uiText('该视图已注册，但当前算例没有匹配结果。','The view is registered, but this case has no matching result.'));
