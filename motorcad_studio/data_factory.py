...
class DataFactoryService:
    ...
    def case_records(self, task_id: str) -> list[dict[str, Any]]:
        ...
        if task.get("execution_plan_id"):
            execution_plan = self.db.query_one(
                "SELECT id,content_hash,schema_version,traceability_status,binding_version,target_motorcad_version FROM execution_plans WHERE id=?",
        ...
        for case in cases:
            ...
            authoritative = bundle.legacy_projection() if bundle is not None else result
            ...
            scalars = authoritative.get("scalars") or {}
            ...
            row: dict[str, Any] = {
                ...
                "execution_plan_hash": (execution_plan or {}).get("content_hash") or task.get("execution_plan_hash"),
                "execution_plan_schema_version": (execution_plan or {}).get("schema_version"),
                "execution_traceability_status": (execution_plan or {}).get("traceability_status"),
                "binding_version": (execution_plan or {}).get("binding_version"),
                "target_motorcad_version": (execution_plan or {}).get("target_motorcad_version"),
    ...
    def build_dataset(self, definition: dict[str, Any]) -> dict[str, Any]:
        ...
        template_ids = {str(row.get("template_id")) for row in rows if row.get("template_id")}
        schema = {
            key: {"type": self._column_type(rows, key), "unit": self._field_unit(key, template_ids=template_ids)}
            for key in fields
        }
        ...
        manifest = {
            ...
            "lineage_policy": "case_id + execution_plan_id/content_hash + run_configuration_id/content_hash compatibility lineage + design/analysis/scenario/solver/result contract hashes + input_hash + native binding provenance",
    ...
    def _field_unit(self, key: str, *, template_ids: set[str] | None = None) -> str | None:
        templates = sorted(template_ids or set())
        if key.startswith("param."):
            parameter_id = key[6:]
            units: set[str] = set()
            if templates:
                for template_id in templates:
                    definition = self.registry.parameter_schema(template_id).get(parameter_id, {})
                    if definition.get("unit"):
                        units.add(str(definition.get("unit")))
            else:
                definition = self.registry.parameter_registry.get(parameter_id, {})
                if definition.get("unit"):
                    units.add(str(definition.get("unit")))
        ...
        if key.startswith("result."):
            ...
            if templates:
                for template_id in templates:
                    definition = self.registry.output_schema(template_id).get(result_id, {})
                    if definition.get("unit"):
                        units.add(str(definition.get("unit")))
            else:
                definition = self.registry.output_registry.get(result_id, {})
                if definition.get("unit"):
                    units.add(str(definition.get("unit")))
            return next(iter(units)) if len(units) == 1 else None
        derived_units = {
