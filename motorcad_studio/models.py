...
class UncertaintyDistributionDefinition(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_distribution(self) -> "UncertaintyDistributionDefinition":
        if self.distribution in {"uniform", "triangular"}:
            if self.lower_delta is None or self.upper_delta is None or self.upper_delta <= self.lower_delta:
                raise ValueError("均匀/三角分布必须满足 lower_delta < upper_delta")
            if self.distribution == "triangular" and self.mode_delta is not None and not (self.lower_delta <= self.mode_delta <= self.upper_delta):
                raise ValueError("三角分布 mode_delta 必须位于上下界之间")
        if self.distribution == "normal" and (self.sigma is None or self.sigma <= 0):
            raise ValueError("正态分布必须设置 sigma > 0")
        return self


class RobustnessDefinition(BaseModel):
    enabled: bool = False
    samples: int = Field(default=8, ge=2, le=256)
    seed: int = 7403
    include_nominal: bool = True
    sampling: Literal["latin_hypercube", "random"] = "latin_hypercube"
    distributions: list[UncertaintyDistributionDefinition] = Field(default_factory=list, max_length=16)
    ...
    @model_validator(mode="after")
    def validate_robustness(self) -> "RobustnessDefinition":
        if self.enabled and not self.distributions:
            raise ValueError("启用鲁棒性分析时至少需要一个不确定性分布")
        if self.enabled and not self.include_nominal:
            raise ValueError("V0.74-C integrated robustness requires include_nominal=true")
        ids=[f"{row.target_scope}:{row.target_id}" for row in self.distributions]
        if len(ids) != len(set(ids)):
            raise ValueError("鲁棒性不确定性目标不能重复")
        return self


class SensitivityDefinition(BaseModel):
    enabled: bool = False
    methods: list[Literal["local", "morris", "sobol"]] = Field(default_factory=lambda: ["local", "morris", "sobol"], max_length=3)
    output_ids: list[str] = Field(default_factory=list, max_length=8)
...
class ExperimentDefinition(BaseModel):
    ...
    robustness: RobustnessDefinition = Field(default_factory=RobustnessDefinition)
    sensitivity: SensitivityDefinition = Field(default_factory=SensitivityDefinition)
...
class ScenarioDefinition(BaseModel):
    ...
    induction_slip: float | None = Field(default=None, ge=-1.0, le=1.0)
...
class MaterialConfiguration(BaseModel):
    ...
    # Preserve design-side material lineage through native validation.  Template
    # inherited assignments are already authoritative inside the loaded .mtt/.mot
    # and should not be re-written merely because the UI displays them.
    material_provenance: dict[str, dict[str, Any]] = Field(default_factory=dict)
    inherited_component_materials: dict[str, str] = Field(default_factory=dict)
    template_component_materials: dict[str, str] = Field(default_factory=dict)
...
class TaskCreate(BaseModel):
    ...
    # V0.73-B: set by the server after freezing the immutable ExecutionPlan.
    # Client-supplied values are verified against the persisted plan before execution.
    ...
    # V0.74 optimization-object compatibility envelope. Server-generated values are frozen before execution.
    ...
    def validate_case_source(self) -> "TaskCreate":
        ...
        if self.scenario_matrix and (self.case_matrix or self.sweep.enabled):
            raise ValueError("分析定义多工况不能与参数矩阵或一维扫描同时启用")
        if self.scenario_matrix and self.experiment.mode != ExperimentMode.SINGLE and not self.operating_point_set:
            raise ValueError("多工况DOE/优化必须冻结 OperatingPointSet")
        return self
...
# V0.73-A canonical names. Historical NativeParity* names remain API-model aliases.
NativeClosureRunRequest = NativeParityRunRequest
NativeClosureSuiteRequest = NativeParitySuiteRequest
...
class DesignStarterCreate(BaseModel):
...
class MotorCADBindingPlanRequest(BaseModel):
    analysis: AnalysisType = AnalysisType.EMAG
    parameters: dict[str, Any] = Field(default_factory=dict)
    explicit_parameter_ids: list[str] = Field(default_factory=list, max_length=512)
    materials: MaterialConfiguration | None = None
    scenario: ScenarioDefinition = Field(default_factory=ScenarioDefinition)
    solver_settings: dict[str, Any] = Field(default_factory=dict)
...
class AnalysisTemplatePreviewRequest(BaseModel):
    design_revision_id: str = Field(min_length=1, max_length=120)
...
class StandardValidationPackageRequest(BaseModel):
    decisions_by_analysis: dict[str, dict[str, Any]] = Field(default_factory=dict)
    notes: str = Field(default="", max_length=4000)


class StandardValidationExecuteRequest(StandardValidationPackageRequest):
...
class AnalysisAutoFixRequest(BaseModel):
    ...
    expected_analysis_revision_id: str = Field(min_length=1, max_length=120)
...
class AnalysisExecutionRequest(BaseModel):
    ...
    expected_analysis_revision_id: str | None = Field(default=None, min_length=1, max_length=120)
    expected_design_revision_id: str | None = Field(default=None, min_length=1, max_length=120)
    expected_execution_plan_hash: str | None = Field(default=None, min_length=16, max_length=128)




class AnalysisOperatingPointSelection(BaseModel):
    load_case_index: int = Field(ge=0, le=4999)
...
class AnalysisExperimentRequest(BaseModel):
    ...
    operating_points: list[AnalysisOperatingPointSelection] = Field(default_factory=list, max_length=128)
    name: str | None = Field(default=None, min_length=1, max_length=120)
    quality_profile: str = Field(default="standard", min_length=1, max_length=80)
    reuse_cache: bool = True
    submission_key: str | None = Field(default=None, min_length=8, max_length=120)
    ...
    expected_design_revision_id: str | None = Field(default=None, min_length=1, max_length=120)
    expected_execution_plan_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_optimization_space_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_experiment_plan_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_operating_point_set_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_uncertainty_scenario_set_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_robustness_plan_hash: str | None = Field(default=None, min_length=16, max_length=128)
...
class CandidateValidationRequest(BaseModel):
    critical_point_count: int = Field(default=3, ge=1, le=16)
...
class OptimizationCandidatePromotionRequest(BaseModel):
    ...
    expected_candidate_validation_report_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_candidate_result_set_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_result_authority_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_robust_candidate_evaluation_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_robust_result_authority_closure_hash: str | None = Field(default=None, min_length=16, max_length=128)
    expected_optimization_decision_snapshot_hash: str | None = Field(default=None, min_length=16, max_length=128)
...
class OptimizationEvidenceLedgerCaptureRequest(BaseModel):
    reason: str = Field(default="manual", max_length=240)


class OptimizationReplayPlanCreateRequest(BaseModel):
...
class OptimizationReplayExecuteRequest(BaseModel):
    force_restart_validation: bool = True
    allow_changed_environment: bool = False
