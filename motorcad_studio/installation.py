...
class MotorCADInstallationManager:
    ...
    @classmethod
    def normalize_motorcad_version(cls, value: str | None) -> str | None:
        """Normalize a Motor-CAD product/file version to the Studio release form.

        Windows executable metadata is commonly exposed as values such as
        ``2026.1.0.0`` while Studio's mapping authority uses ``2026R1``.  Keep the
        raw metadata for evidence and derive the normalized release explicitly so
        production qualification never has to infer a release from an unrelated
        preflight ``version`` field.
        """
        if not value:
            return None
        return cls._version_from_path(Path(str(value)))

    @staticmethod
    def _windows_executable_version_info(path: Path, timeout_s: float = 20.0) -> dict[str, Any]:
        """Read immutable Windows file-version metadata for a Motor-CAD executable.

        The acceptance workstation already requires PowerShell for orchestration.
        Reading ``VersionInfo`` here avoids treating the installation-directory
        name as proof of the binary version.  Failures are returned as evidence;
        they are never silently promoted to a version match.
        """
        if platform.system() != "Windows" or not path.is_file():
            return {"available": False, "reason": "windows_executable_required"}
        ...
        script = (
            f"$v=(Get-Item -LiteralPath '{literal}').VersionInfo; "
            "$o=[ordered]@{FileVersion=$v.FileVersion; ProductVersion=$v.ProductVersion; "
            "ProductName=$v.ProductName; CompanyName=$v.CompanyName; OriginalFilename=$v.OriginalFilename}; "
        ...
        try:
            ...
            return {"available": False, "reason": "version_info_timeout"}
        except Exception as exc:
            return {"available": False, "reason": "version_info_error", "error": f"{type(exc).__name__}: {exc}"}
        if proc.returncode != 0:
            return {
                "available": False,
                "reason": "version_info_process_failed",
        ...
        try:
            ...
            return {"available": False, "reason": "version_info_invalid_json", "error": f"{type(exc).__name__}: {exc}"}
        return {
            "available": True,
            "file_version": str(payload.get("FileVersion") or "").strip() or None,
            "product_version": str(payload.get("ProductVersion") or "").strip() or None,
            "product_name": str(payload.get("ProductName") or "").strip() or None,
            "company_name": str(payload.get("CompanyName") or "").strip() or None,
            "original_filename": str(payload.get("OriginalFilename") or "").strip() or None,
        }

    def executable_identity(self, exe_path: str | None = None) -> dict[str, Any]:
        ...
        path_version = self._version_from_path(path)
        metadata = self._windows_executable_version_info(path)
        file_version = metadata.get("file_version")
        product_version = metadata.get("product_version")
        normalized = self.normalize_motorcad_version(product_version) or self.normalize_motorcad_version(file_version)
        return {
            "available": bool(path.is_file() and metadata.get("available")),
            "exe_path": str(path.resolve()) if path.exists() else str(path),
            "exists": path.is_file(),
            "path_inferred_version": path_version,
