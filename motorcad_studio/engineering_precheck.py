...
def validate_engineering_inputs(
    ...
    *,
    ...
    rotor = _number(_first(flat, keys=("rotor_outer_diameter", "rotor_diameter", "rotor_dia")))
    ...
    if shaft is not None and rotor is not None and shaft >= rotor:
    ...
    if rotor is not None and bore is not None and rotor >= bore:
        add("GEOM_ROTOR_INSIDE_BORE", "BLOCKING", "geometry", "转子外径必须小于定子内径。", ["rotor_diameter", "stator_inner_diameter"], "减小转子外径或增大定子内径。")
    if rotor is not None and bore is not None and gap is not None and bore - rotor < 2 * gap - 1e-9:
    ...
    if magnet is not None and shaft is not None and rotor is not None and 2 * magnet >= rotor - shaft:
    ...
    if outer is not None and bore is not None and slot_depth is not None and slot_depth >= (outer - bore) / 2:
