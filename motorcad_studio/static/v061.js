/* MotorCAD Studio V0.60.1 repair — local Motor-CAD material library management. */
(() => {
  ...
  function newRecord(){stateV061.selected={id:null,name:'',kind:'solid',material_type:'General',source_kind:'studio_custom',properties:{Type:'Fixed_Solid','Solid Type':'General'},summary:{bh_curve:[],loss_points:[],temperature_curves:{}}};renderList();renderDetail();setTimeout(()=>q('#materialNameV061')?.focus(),0)}
  ...
  function renderShellBody(){
    ...
    body.innerHTML=`${stateV061.picker?`<div class="material-picker-banner-v062"><b>${safe(stateV061.picker.componentLabel||'当前部件')}</b><span>请选择 ${safe(stateV061.picker.materialType||'合适类型')} 材料，确认后返回设计草稿。</span></div>`:''}<div class="material-library-intro-v061"><div><span>本机数据优先</span><h3>以当前 Motor-CAD 安装实际数据库为材料事实源</h3><p>扫描会读取默认文件位置和可发现的 Solids.mdb / Fluids.mdb。导入内容作为只读快照；工程修改进入 Studio 管理副本，避免覆盖 Motor-CAD 原始材料库。</p></div><div class="material-library-counts-v061"><div><span>材料记录</span><b>${stateV061.status?.records??0}</b></div><div><span>Studio 管理</span><b>${stateV061.status?.custom_records??0}</b></div><div><span>Motor-CAD</span><b>${safe(stateV061.status?.motorcad_version||'—')}</b></div></div></div>
  ...
  async function refreshRecords(preferredId=null){const params=new URLSearchParams();if(stateV061.query)params.set('q',stateV061.query);if(stateV061.kind)params.set('kind',stateV061.kind);if(stateV061.materialType)params.set('material_type',stateV061.materialType);params.set('limit','1500');try{const result=await api(`/api/material-library?${params.toString()}`);stateV061.records=result.records||[];const nextId=preferredId||(stateV061.selected?.id&&stateV061.records.some(row=>row.id===stateV061.selected.id)?stateV061.selected.id:null);if(nextId){stateV061.selected=await api(`/api/material-library/${encodeURIComponent(nextId)}`)}else if(stateV061.selected?.id&&!stateV061.records.some(row=>row.id===stateV061.selected.id)){stateV061.selected=null}renderList();renderDetail()}catch(error){toast(`材料列表读取失败：${error.message}`,'ERROR')}}
