/* MotorCAD Studio V0.73-D — Design Revision horizontal comparison. */
(() => {
  ...
  function renderSetup(){
    ...
    q('[data-revision-run-v069]',host)?.addEventListener('click',()=>{
      ...
      const target=window.MCSResultsWorkbench?.projectPath?.(suffix);
      if(target&&location.pathname!==target)return window.MCSResultsWorkbench?.navigate?.(target);
      return runCompare();
    });
  ...
  function renderResult(payload){
    ...
    const setAggregate=payload.result_set_aggregate||null,setGate=setAggregate?.comparability||{};
    const formal=Boolean(payload.formal_results_comparable);
    const gateIssues=[...(setGate.blocking_issues||[]),...(setGate.review_issues||[])];
    box.innerHTML=`<div class="comparison-formal-gate-v073d ${formal?'ready':'pending'}"><b>${formal?'正式性能比较 Gate 通过':'性能结果当前仅可审查'}</b> · ${formal?'ResultSet Aggregate 已确认 Analysis Revision、工况、Solver 与 Trust 满足正式比较。':payload.results_comparable?'同条件指标可比较，但当前 Trust 或工程 Gate 尚未完成正式资格。':'各 Revision 的分析/工况/求解条件不一致。'}${setAggregate?`<small> · ResultSetAggregateV1 ${safe(setAggregate.contract_version||'0.79-B')} · ${Number(setGate.comparable_metric_count||0)} metrics${gateIssues.length?` · ${safe(gateIssues.join(' / '))}`:''}</small>`:''}</div><div class="revision-comparison-summary-v069">
    ...
    <section><div class="section-head"><div><h3>最近可用结果证据</h3><p>每个 Revision 只取最近一次 COMPLETED/PARTIALLY_COMPLETED 任务中的首个 VALID/WARNING Case，避免自动拼接不同工况。</p></div></div><div class="comparison-table-scroll-v069"><table><thead><tr><th>证据</th>${headerCells(revisions)}</tr></thead><tbody>
      ...
      <tr><td>Result Trust</td>${revisions.map(row=>{const trust=evidence.get(row.id)?.trust;return `<td class="revision-trust-cell-v073d">${trust?(window.MCSResultsTrust?.renderBadge?.(trust)||safe(trust.engineering_status)):'—'}<small>${safe(evidence.get(row.id)?.result_authority||'')}</small></td>`}).join('')}</tr>
      <tr><td>Result Bundle</td>${revisions.map(row=>`<td>${safe(String(evidence.get(row.id)?.result_bundle_hash||'—').slice(0,18))}<small>${safe(evidence.get(row.id)?.result_authority||'')}</small></td>`).join('')}</tr>
    ...
    qa('[data-open-revision-v069]',box).forEach(button=>button.addEventListener('click',()=>window.MCSResultsWorkbench?.navigate?.(`/app/projects/${encodeURIComponent(currentProjectId())}/designs/${encodeURIComponent(payload.design.id)}/revisions/${encodeURIComponent(button.dataset.openRevisionV069)}/geometry/radial`)));
  }
  ...
  const apiController=Object.freeze({mount,runCompare,state:controller});
  window.MCSRevisionCompare=apiController;
})();
