...
(() => {
  ...
  function resetDialogSize(section){if(!section)return;section.style.width='80vw';section.style.height='80vh'}
  ...
  function shell(){
    ...
    node.innerHTML=`<div class="material-library-backdrop-v061"></div><section role="dialog" aria-modal="true" aria-label="Motor-CAD 材料库"><header><div><span>${picker?'材料选择':'材料工程数据'}</span><h2>${safe(picker?.title||'Motor-CAD 材料库')}</h2><p>${picker?'单击预览，双击直接赋值；材料库记录本身不会因赋值而修改。':'读取本机实际 .mdb，保留来源与文件哈希；编辑导入材料时创建 Studio 管理副本。'}</p></div><button type="button" data-material-close-v061 aria-label="关闭">×</button></header><div id="materialLibraryBodyV061" class="material-library-body-v061"><div class="material-loading-v061">正在读取材料数据库状态…</div></div><div class="material-library-resize-handle-v088" data-material-resize-v088 title="拖动调整窗口大小；双击恢复 80%"></div></section>`;
  ...
  function materialCurvePreview(summary){
    ...
    if(magnet.length){
      ...
      charts.push(curveChart(derived.map(row=>({x:row.h,y:row.b})),`磁体退磁工作线${target===undefined?'':` · ${target} °C`}（推导）`,'H / A·m⁻¹','B / T'));
  ...
  function renderDetail(){
    ...
    box.innerHTML=`<div class="material-detail-head-v061"><div><span>${isNew?'新建材料':safe(sourceLabel(row))}</span><h3>${safe(row.name||'未命名材料')}</h3><p>${picker?'正在预览材料；确认赋值只会更新当前部件的设计草稿。':imported?'当前记录是本机 Motor-CAD 数据库的只读快照。修改并保存后会创建 Studio 管理副本，源 .mdb 不会被写入。':'当前记录由 Studio 管理；导出管理数据库后可在计算配置中选用。'}</p></div><div class="material-detail-actions-v061">${!picker&&row.id?`<button type="button" data-material-clone-v061>复制为新材料</button><button type="button" data-material-delete-v061 class="danger">${imported?'从 Studio 索引移除':'删除材料'}</button>`:''}</div></div>
  ...
  function renderShellBody(){
    ...
    const intro=picker?'':`<div class="material-manager-mode-v066"><b>材料管理模式</b><span>这里用于检索、查看曲线和维护数据库，不会直接改动当前 Design Revision。应用材料请回到“材料配置”选择部件。</span></div><div class="material-library-intro-v061"><div><span>本机数据优先</span><h3>以当前 Motor-CAD 安装实际数据库为材料事实源</h3><p>扫描会读取默认文件位置和可发现的 Solids.mdb / Fluids.mdb。导入内容作为只读快照；工程修改进入 Studio 管理副本，避免覆盖 Motor-CAD 原始材料库。</p></div><div class="material-library-counts-v061"><div><span>材料记录</span><b>${stateV061.status?.records??0}</b></div><div><span>Studio 管理</span><b>${stateV061.status?.custom_records??0}</b></div><div><span>Motor-CAD</span><b>${safe(stateV061.status?.motorcad_version||'—')}</b></div></div></div><section class="material-source-section-v061"><header><div><h3>数据库来源</h3><p>优先扫描 Motor-CAD Defaults.INI 指向的实际数据库，也支持手工输入 .mdb 路径。</p></div><button type="button" class="primary" data-material-scan-v061>扫描本机 Motor-CAD 数据库</button></header><div class="material-import-row-v061"><input id="materialDatabasePathV061" placeholder="例如 C:\\Users\\...\\Solids.mdb"><button type="button" data-material-import-v061>按路径导入</button></div><div class="material-source-grid-v061">${sourceCards()}</div></section>`;
