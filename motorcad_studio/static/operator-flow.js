...
(() => {
  ...
  function syncProjectStageStateV019() {
    ...
    const stageState = {
      ...
      motor: designCount > 0 ? {status:'ready', title:'配置方案的几何、绕组、材料和 Revision'} : {status:'pending', title:'需要先创建方案'},
  ...
  function syncProjectShellV019(tab = activeTabV019()) {
    ...
    if (p && $('#currentProjectWorkspaceMeta')) $('#currentProjectWorkspaceMeta').textContent = `${p.id} · 当前方案的几何、绕组、材料和不可变 Revision 在这里维护；工况与求解设置进入“分析配置”。`;
  ...
  function chooseNextActionV019({dashboard, project, readiness}) {
    ...
    if (!designs.length) return {tab:'solutions', title:'创建第一个方案', detail:'先建立方案身份，再进入电机配置维护几何、绕组、材料和不可变 Revision。', button:'进入方案管理', short:'项目尚无方案'};
    ...
    if (!completed) return {tab:'analysisConfig', title:'创建第一条基准分析', detail:'选择已保存的电机 Revision，配置工况、分析类型和输出。基准成功后再做扫描、DOE 或优化。', button:'进入分析配置', short:'尚无成功基准任务'};
