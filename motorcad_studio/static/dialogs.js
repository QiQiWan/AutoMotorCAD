...
(() => {
  ...
  function close(id,value=false){const el=document.getElementById(id);if(!el)return;el.classList.add('closing');const remove=()=>{if(el.isConnected)el.remove()};el.addEventListener('transitionend',remove,{once:true});requestAnimationFrame(()=>{if(getComputedStyle(el).transitionDuration==='0s')remove()});const resolve=el._resolve;if(resolve){el._resolve=null;resolve(value)}}
