...
(() => {
  ...
  function designStageForView(view){return window.MCSAppCoreV062?.stageForView?.(view)||(view==='winding'||view==='slot'?'winding':view==='materials'?'materials':['evidence','native'].includes(view)?'validation':view==='compare'?'compare':'geometry')}
  ...
  function renderDesignView(){
    ...
    window.MCSAppCoreV062?.emit?.('mcs:workspace-rendered',{designId:visualState.designId,revisionId:visualState.revisionId,view:visualState.view,editing:false});
  }
  ...
  function bindDesignViewer(root){
    ...
    root.addEventListener('click',event=>{
      ...
      const next=event.target.closest('[data-design-next-v061]');if(next){const target=next.dataset.designNextV061;if(target==='input_data'){window.MCSV060?.openInputCenter?.();return;}setView(target,{source:'viewer-next'});return;}
  ...
  function applyRouteView(route){
    let requested=route?.designView||window.MCSAppCoreV062?.viewForRoute?.(route?.designSection,route?.designSubview);if(requested==='native')requested='evidence';if(!requested)return;
  ...
  if(typeof previousOpenWorkspaceDesign==='function')window.openWorkspaceDesign=async function(){
  ...
  if(typeof previousSelectWorkspaceRevision==='function')window.selectWorkspaceRevision=function(){
  ...
  window.MCSDesignViewer={state:visualState,decorate:decorateDesignViewer,render:renderDesignView,applyRouteView,setView,abort:abortPending};
