...
(() => {
  ...
  function windingView(ctx){
    ...
    const slots=clamp(Math.round(number(values.slot_count,12)),3,72),phases=clamp(Math.round(number(w.phase_count||derived.phase_count,3)),1,15),paths=clamp(Math.max(1,Math.round(number(values.parallel_paths,w.parallel_paths||1))),1,24);
  ...
  function slotView(ctx){
    const {data,values}=viewData(ctx),w=data.winding_design||{},editable=Boolean(ctx.editable);
    ...
    const opening=Math.max(.05,number(values.slot_opening,2)),slotWidth=Math.max(opening+.1,number(values.slot_width,Math.max(opening*3,6))),depth=Math.max(1,number(values.slot_depth,16)),tooth=Math.max(.1,number(values.tooth_width,4.8)),corner=Math.max(0,number(values.slot_corner_radius,1.5)),tipDepth=Math.max(0,number(values.tooth_tip_depth,1)),tipAngle=number(values.tooth_tip_angle,0);
