...
(() => {
  ...
  const viewData=ctx=>{const source=ctx.data||{},data=ctx.materials?{...source,materials:ctx.materials}:source,values=ctx.values||source.effective_parameters||{},motorObject=window.MCSMotorObject?.resolve?.(data,values,ctx.materials)||window.MCSPMMotorObject?.resolve?.(data,values,ctx.materials)||legacyMotorObjectAdapter(data,values,ctx.materials);return{data,values,precheck:ctx.precheck||source.precheck||{},editable:Boolean(ctx.editable),selected:ctx.selected||null,motorObject}};
