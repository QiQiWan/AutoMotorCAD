...
(() => {
  ...
  function materialsView(ctx){
    const {data,editable}=viewData(ctx),materials=data.materials||{},components=materials.component_materials||{},provenance=materials.material_provenance||{},inherited=materials.inherited_component_materials||{};
    ...
    const rows=defs.map(row=>{
      const value=valueFor(components,row),origin=originFor(provenance,row),isTemplate=origin.source_kind==='template_mtt'||inherited[row.key]!==undefined,source=origin.source_database_path||origin.source_database||origin.source_template_id||(isTemplate?'模板 MTT':'Motor-CAD / 设计版本'),sectionHash=origin.material_section_hash||'';
      return`<tr><td><b>${safe(row.label)}</b><small>${safe(row.key)}</small></td><td><strong>${safe(materialDisplayName(value))}</strong>${isTemplate?'<small class="material-template-default-v066">模板默认</small>':''}</td><td><span title="${safe(source)}">${safe(String(source).split(/[\\/]/).pop()||source)}</span>${sectionHash?`<small title="${safe(sectionHash)}">材料段 ${safe(String(sectionHash).slice(0,12))}…</small>`:''}</td><td>${editable?`<button type="button" class="material-apply-button-v066" data-workbench-material-component="${safe(row.key)}" data-material-type-v062="${safe(row.type)}">${value?'选择其他材料':'选择材料'}</button>`:`<span class="material-state-v031">${isTemplate?'模板基线':value?'版本冻结':'模型继承'}</span>`}</td></tr>`;
    }).join('');
    return`<div class="motorcad-view-v031 materials-view-v031 materials-assignment-v062"><div class="visual-heading-v031"><div><span class="eyebrow">DESIGN · MATERIAL ASSIGNMENT</span><h3>部件材料配置</h3><p>优先继承模板自身材料定义，通常无需逐项重选。只有需要替换某个部件时，点击该行“选择其他材料”。</p></div><div class="visual-facts-v031"><span>${assigned} 项已有材料</span><span>${inheritedCount} 项模板默认</span><span>${defs.length} 个核心部件</span></div></div>
      ...
      <div class="materials-meta-v031 materials-library-entry-v061"><div><span>Motor-CAD 材料工程数据库</span><b>${safe(materials.material_database_path||'当前模型 / 默认材料数据库')}</b><small>材料库用于检索、曲线和原始属性管理；当前表用于把具体材料绑定到 Design Revision。</small></div><button type="button" data-open-material-library-v061>管理材料库 / 查看曲线</button></div>
      <div class="analysis-boundary-v062"><div><b>冷却介质属于分析工况</b><span>水 / 油 / 空气、入口温度、流量和对流边界在分析案例中配置，可对同一 Design Revision 建立多个冷却方案。</span></div><button type="button" data-design-next-v061="input_data">进入分析设置 →</button></div>${authorityStrip('Design Revision 部件材料快照；模板默认材料在新 Revision 保存时冻结')}
    </div>`;
  }
