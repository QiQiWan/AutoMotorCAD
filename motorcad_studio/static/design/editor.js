...
(() => {
  ...
  function updateChangeCount() {
    ...
    const save = $q('#workbenchSaveV024'); if (save) save.disabled = wb.saveBusy || count === 0 || Boolean(draftConflict());
  ...
  function nativeFailureSummary(result) {
    ...
    return `<div class="native-failure-summary-v024"><span>${safe(causes[0] || result?.geometry?.message || result?.winding?.message || 'Motor-CAD 返回模型不可行')}</span>${binding.length ? `<div>${binding.map(id => `<button type="button" data-workbench-select="${safe(id)}">定位 ${safe(parameterLabel(id))}</button>`).join('')}</div>` : ''}</div>`;
  }
  ...
  function renderEvidence() {
    ...
    if (check.nativeBusy) nativePart = '<div class="native-check-result-v024 history"><b>Motor-CAD 原生检查正在运行…</b></div>';
      ...
      nativePart = `<div class="native-check-result-v024 ${pass ? 'pass' : 'fail'}"><b>${pass ? '✓ 当前草稿已通过 Motor-CAD 原生几何与绕组检查' : '当前草稿未通过 Motor-CAD 原生模型检查'}</b>${pass ? '' : nativeFailureSummary(native)}</div>`;
    ...
    box.innerHTML = `<span class="eyebrow">模型检查记录</span><h3>${previous?.source === 'revision' ? `上一可行版本 ${safe(previous.revision)}` : previous ? '起始模板可行' : '等待首次检查'}</h3>${nativePart}<div class="evidence-rule-v024"><b>验证顺序</b><span>Studio 参数关系 → Motor-CAD 几何与绕组 → 分析工况检查 → 正式计算 → 结果验证</span></div>`;
  }
  ...
  async function runNativeCheck() {
    try {
      ...
      if (!result) return;
      toast(result.status === 'PASS' ? '当前草稿已通过 Motor-CAD 原生几何与绕组检查。' : 'Motor-CAD 原生模型检查未通过；已显示对应工程约束。', result.status === 'PASS' ? 'SUCCESS' : 'ERROR', 7000);
    } catch (error) {
  ...
  async function saveRevision() {
    ...
    const button = $q('#workbenchSaveV024'); if (button) button.textContent = '正在冻结 Revision…';
    try {
      ...
      const created = await api(`/api/designs/${encodeURIComponent(design.id)}/draft/commit`, {method: 'POST', body: JSON.stringify({expected_version: expectedVersion, notes: notes || `基于 Rev.${state.workspaceRevision?.revision} 的设计草稿`, analysis_definition_id: analysisDefinitionId})});
      ...
      document.body.classList.remove('design-editing-v062'); window.MCSDesignStore?.setMode?.('read', {source: 'editor-commit'});
      if (window.MCSV060?.state) window.MCSV060.state.fetchedAt = 0;
      toast(`已创建 Rev.${created.revision}；${created.linked_analysis_definition_id ? '当前分析案例已引用新版本；' : ''}草稿已清除。`, 'SUCCESS', 6500);
      await openWorkspaceDesign(design.id); if (created.id) selectWorkspaceRevision(created.id);
      window.MCSRouter?.setRevisionEditMode?.(false, {view: wb.view, replace: true}); window.MCSRouter?.syncDesignView?.(wb.view, {replace: true});
    } catch (error) {
      ...
      toast(error.message, 'ERROR', 8500); wb.saveBusy = false; if (button) button.textContent = '保存为新 Revision'; updateChangeCount();
    }
  }
  ...
  function bindShell() {
    ...
    $q('#workbenchOpenParameterCatalogV066')?.addEventListener('click', () => { if (totalChangeCount()) { toast('当前草稿已有修改。请先保存为 Revision 或放弃草稿，再进入全量 Motor-CAD 参数目录，避免产生并行版本分支。', 'WARNING', 7000); return; } window.MCSV040?.openParameterCatalog?.(); }, {signal});
  ...
  async function open(routeCtx = null) {
    ...
    try {
      ...
      const [data, draftResult] = await Promise.all([
        ...
        api(`/api/designs/${encodeURIComponent(design.id)}/draft`, options).catch(() => ({exists: false, draft: null})),
      ]);
