...
(() => {
  ...
  function create(options = {}) {
          ...
          try {
            if (request.deleteDraft) {
              if (request.force || service.draft) {
                ...
                await api(`/api/designs/${encodeURIComponent(request.designId)}/draft?expected_version=${encodeURIComponent(deleteVersion)}`, {method: 'DELETE'});
              }
              ...
              const result = await api(`/api/designs/${encodeURIComponent(request.designId)}/draft`, {
                method: 'PUT',
    ...
    function schedule({delay = 650, reason = 'autosave'} = {}) {
      clearTimeout(service.timer);
      if (hasChanges()) service.lastSavedAt = null;
      service.lastError = null;
      notify();
