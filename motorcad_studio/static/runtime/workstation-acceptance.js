(function(global){
  ...
  function renderV089D(payload,nativePayload,box,badge){
    ...
    if(badge){badge.textContent=qualified?'V0.89-D 正式通过':(nativeQualified?'Native通过 · 待UI旅程':'待工作站验收');badge.className=`badge ${qualified?'VALID':'UNVERIFIED'}`}
    ...
    const steps=[
      ['1 · V0.88-F Native底座',nativeQualified?`${native.passed}/${native.required} Native · ${native.faultPassed}/${native.faultRequired} 故障`:'待 4/4 Native + 17/17 故障',nativeQualified],
      ...
      ['6 · V0.89 A/B/C Gate',coverage.release_gate_passed===coverage.release_gate_required?`${coverage.release_gate_passed||0}/${coverage.release_gate_required||3}`:'待完整发布门禁',coverage.release_gate_required>0&&coverage.release_gate_passed===coverage.release_gate_required],
    ];
    box.innerHTML=`<div class="workstation-acceptance-overview ${qualified?'qualified':'pending'}"><div><span class="eyebrow">V0.89-D WINDOWS NATIVE GOLDEN JOURNEY</span><h3>${qualified?'真实工作站与 UI Golden Journey 已正式通过':'V0.89-D 正式工作站资格尚未完成'}</h3><p>${qualified?'SPM、IPM、AFPM 已在真实 Windows + licensed Motor-CAD 2026R1 上经完整 Studio UI 创建、预检、求解并重新打开结果。':'V0.89-D 在 V0.88-F Native 资格之上再要求 3/3 真实完整界面 Golden Journey；模拟 E2E 不能提升正式资格。'}</p></div><div class="workstation-qualification-score"><b>${payload.qualification_percent||0}%</b><small>V0.89-D Formal</small><span>${evidenceCoverage.toFixed(0)}% evidence</span></div></div><div class="workstation-onboarding-grid">${steps.map(([label,value,ok])=>`<div class="workstation-onboarding-step ${ok?'ready':'pending'}"><span>${esc(label)}</span><b>${esc(value)}</b></div>`).join('')}</div><div class="workstation-matrix"><div class="workstation-matrix-head"><b>3 条真实 UI Golden Journey</b><span>SPM / IPM / AFPM</span></div>${journeyRows||'<div class="workstation-matrix-row pending"><b>Pending</b><span>尚无真实 UI Journey evidence</span></div>'}</div>${blockers.length?`<div class="workstation-acceptance-blockers"><b>V0.89-D 当前资格阻断</b>${blockers.slice(0,12).map(x=>`<span>${esc(x)}</span>`).join('')}</div>`:''}<div class="workstation-acceptance-command">run_windows_production_qualification.bat</div>`;
    return payload;
  }
