...
(() => {
  ...
  const CONTRACT_VERSION = '0.89-G2';
  ...
  const RULES = [
    ...
    },
    {
      ...
      evaluate:()=>{const a=analysis();if(!a.active)return blocked('请先创建或选择分析配置。',recovery.click('#analysisCreateV076','新建分析配置'),'ANALYSIS_REQUIRED');if(!a.executionPlan)return blocked('完整计算前检查需要先加载执行计划。',q('#analysisInitialPlanV076')?recovery.click('#analysisInitialPlanV076','加载执行计划'):recovery.click('#analysisRefreshPlanV076','刷新执行计划'),'EXECUTION_PLAN_REQUIRED');return ready('可以运行完整计算前检查。')}
    },
    {
      id:'ANALYSIS_SUBMIT', selector:'#analysisSubmitV076',
      evaluate:()=>{const a=analysis(),plan=a.executionPlan,full=a.fullCheck;if(!a.active)return blocked('请先创建或选择分析配置。',recovery.click('#analysisCreateV076','新建分析配置'),'ANALYSIS_REQUIRED');if(!plan)return blocked('尚未加载当前分析的执行计划。',q('#analysisInitialPlanV076')?recovery.click('#analysisInitialPlanV076','加载执行计划'):recovery.click('#analysisRefreshPlanV076','刷新执行计划'),'EXECUTION_PLAN_REQUIRED');if(!plan.can_submit){const first=q('[data-fix-first-v081a]');return blocked('当前分析仍有阻断项，尚不能提交计算。',first?recovery.click('[data-fix-first-v081a]','处理第一个阻断项'):recovery.click('#analysisRefreshPlanV076','刷新并查看阻断项'),'ANALYSIS_BLOCKED')}if(!full?.valid)return blocked('Studio 配置已就绪，还需要完成 Motor-CAD 完整计算前检查。',recovery.click('#analysisFullCheckV076','运行完整计算前检查'),'FULL_NATIVE_CHECK_REQUIRED');return ready('当前执行计划和 Motor-CAD 检查证据均已就绪。')}
    },
    {
