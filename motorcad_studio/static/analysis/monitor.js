...
(() => {
  ...
  const revisionLabel=(value,kind='analysis')=>window.MCSAnalysisLabels?.revisionLabel?.(value,kind)||String(value??'—');
  ...
  async function decorate(taskId){
    ...
    node.innerHTML='<div class="analysis-monitor-loading-v067"><span class="spinner-dot"></span>正在读取当前分析与电机版本…</div>';
    try{
    ...
    }catch(error){if(token!==requestToken)return;node.innerHTML=`<div class="analysis-monitor-loading-v067 warning">当前任务的分析版本暂时无法读取：${safe(error.message)}</div>`}
  }
