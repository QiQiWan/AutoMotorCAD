...
(() => {
  ...
  async function decorate(taskId){
    ...
    try{
      ...
      q('[data-analysis-monitor-results-v067]',node)?.addEventListener('click',()=>{const pid=row.project_id||state.activeProjectId;if(window.MCSRouter?.navigate&&pid)return MCSRouter.navigate(`/app/projects/${encodeURIComponent(pid)}/results/tasks/${encodeURIComponent(taskId)}`);showTab('resultViewer')});
  ...
  window.MCSAnalysisMonitorV067={decorate};
})();
