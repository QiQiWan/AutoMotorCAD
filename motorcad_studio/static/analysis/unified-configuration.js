/* V0.81-C Analysis Templates + Smart Defaults + Auto-fix HMI convergence.
 *
...
(() => {
  ...
  const text={
    ...
    loadFailed:'\u5206\u6790\u914d\u7f6e\u52a0\u8f7d\u5931\u8d25',
    saved:'\u5df2\u4fdd\u5b58\u5e76\u751f\u6210\u65b0\u7684 Analysis Revision',
    create:'\u65b0\u5efa\u5206\u6790\u914d\u7f6e',
  };
  ...
  async function fetchCatalogForRevision(revisionId=ctl.selectedRevisionId){
    const record=ctl.revisionIndex.get(String(revisionId||''));
    if(!record){ctl.catalog={recipes:[],modules:[]};return ctl.catalog}
  ...
  async function fetchTemplateCatalogForRevision(revisionId=ctl.selectedRevisionId){
    if(!revisionId||!guidanceClient()){ctl.templateCatalog={templates:[],policy:{common_mode_max_decisions:3}};return ctl.templateCatalog}
  ...
  function renderContext(){
    ...
    root.innerHTML=`<div class="analysis-context-chain-v076">
      <div class="analysis-context-node-v076"><span>Project</span><b>${safe(ctl.project?.name||state.activeProjectId||'-')}</b></div><i class="analysis-context-arrow-v076">\u203a</i>
      <div class="analysis-context-node-v076"><span>Solution</span><b>${safe(activeRecord?.design?.name||'-')}</b></div><i class="analysis-context-arrow-v076">\u203a</i>
      <div class="analysis-context-node-v076"><span>Motor Revision</span><b>${activeRecord?`Rev.${safe(activeRecord.revision?.revision)} \u00b7 ${safe(activeRecord.revision?.id)}`:'-'}</b></div><i class="analysis-context-arrow-v076">\u203a</i>
      <div class="analysis-context-node-v076"><span>Analysis</span><b>${safe(ctl.active?.name||'\u5c1a\u672a\u9009\u62e9')}</b></div>
  ...
  function setStep(step,{replace=true}={}){
    if(!steps.includes(step)||!ctl.active)return;ctl.step=step;renderSteps();renderEditorBody();renderSummary();
  ...
  function renderCreateForm(){
    ...
    body.innerHTML=`<section class="analysis-editor-section-v076"><header><div><span class="eyebrow">${safe(text.create)} · 工程模板</span><h3>${safe(record.design.name||record.design.id)} · Rev.${safe(record.revision.revision)}</h3><p>选择工程意图后，Studio 从当前 Motor Revision、模板和配方生成可追溯 Smart Defaults。常用模式最多只要求 ${safe(limit)} 个关键工程决策。</p></div></header>
      ...
      <div class="analysis-guidance-preview">
        <div class="analysis-create-form-v076"><label>分析配置名称<input id="analysisCreateNameV076" value="${safe(nameDefault)}"></label><div class="analysis-definition-card-v076"><span>电机对象</span><b>${safe(designLabel(record))}</b><small>Motor Revision 是 Smart Defaults 的首要设计证据</small></div><button id="analysisConfirmCreateV076" type="button" class="primary" ${ready?'':'disabled'}>${ctl.templatePreviewLoading?'正在生成推荐…':'创建 Analysis Rev.1'}</button><div id="analysisCreateStatusV076" class="analysis-inline-status-v076">${preview?.error?safe(preview.error):preview&&!ready?'仍有关键工程决策需要确认。':''}</div></div>
        <div class="analysis-guidance-panel"><header><div><span class="chip">${safe(selected?.module||'')}</span><h4>${safe(selected?.label||'请选择分析模板')}</h4><p>${safe(selected?.intent||'模板会把工程目标映射到底层 Motor-CAD 配方。')}</p></div>${preview?`<span class="analysis-confidence ${ready?'high':'medium'}">${ready?'可创建':'待确认'}</span>`:''}</header>
          ${ctl.templatePreviewLoading?'<div class="analysis-list-empty-v076">正在根据当前 Motor Revision 生成推荐值与来源证据…</div>':preview?`<div class="analysis-guidance-decisions">${decisions.map(row=>guidanceClient()?.decisionControl(row)||'').join('')||'<p class="muted">该模板不需要额外关键决策。</p>'}</div>${fixed.length?`<div class="analysis-guidance-recs"><h5>自动配置</h5>${fixed.map(row=>guidanceClient()?.recommendationRow(row)||'').join('')}</div>`:''}${domainDefaults.length?`<div class="analysis-guidance-domain-defaults"><h5>创建时预填的物理输入</h5><p>这些值来自物理输入域默认值，创建后应结合真实材料与冷却边界确认。</p>${domainDefaults.map(row=>`<div><b>${safe(row.label||row.domain_id)}</b><span>${safe(row.field_count)} 个字段 · ${safe(row.confidence_label)}置信度</span><small>${safe(row.reason||'')}</small></div>`).join('')}</div>`:''}`:'<div class="analysis-list-empty-v076">选择模板后生成 Smart Defaults。</div>'}
        </div>
      </div></section>`;
  ...
  function guidanceOverviewHtml({actions=false}={}){
    ...
    return `<div class="analysis-guidance-panel"><header><div><span class="chip">Analysis Guidance</span><h4>${safe(template?.label||'当前分析建议')}</h4><p>${safe(template?.intent||'根据当前 Motor Revision 与 Analysis Revision 生成可追溯建议。')}</p></div><span class="analysis-confidence high">Rev.${safe(guidance.analysis_revision??'-')}</span></header><div class="analysis-guidance-summary"><div><span>关键决策已确认</span><b>${safe(summary.configured_common_decisions??0)}</b></div><div><span>可推荐</span><b>${safe(summary.recommended_common_decisions??0)}</b></div><div><span>待人工确认</span><b>${safe(summary.needs_input_common_decisions??0)}</b></div><div><span>可自动修复</span><b>${safe(summary.applicable_auto_fixes??0)}</b></div></div>${common.length?`<div class="analysis-guidance-recs">${common.map(row=>guidanceClient()?.recommendationRow(row)||'').join('')}</div>`:''}${actions&&fixes.length?`<div class="analysis-guidance-actions"><h5>可执行修复</h5>${fixes.map(row=>guidanceClient()?.actionCard(row)||'').join('')}</div>`:''}</div>`;
  }
  ...
  async function applyAutoFix(actionId){
    ...
    try{
      ...
      renderAll();
      notify(result.idempotent_replay?'已恢复先前完成的 Auto-fix':'Auto-fix 已生成新的 Analysis Revision','SUCCESS',7500);
    }catch(error){
  ...
  function renderDefinition(){
    ...
    body.innerHTML=`<section class="analysis-editor-section-v076"><header><div><span class="eyebrow">1 \u00b7 ${safe(stepLabels.definition)}</span><h3>${safe(ctl.active.name)}</h3><p>\u5206\u6790\u7c7b\u578b\u4e0e\u914d\u65b9\u4f5c\u4e3a\u5206\u6790\u8eab\u4efd\u4fdd\u6301\u7a33\u5b9a\uff1b\u53ef\u4ee5\u5728\u540c\u4e00\u65b9\u6848\u5185\u5207\u6362\u7535\u673a Revision\u3002</p></div><div class="actions"><button type="button" data-go-motor-v076>\u67e5\u770b\u7535\u673a\u914d\u7f6e</button><button type="button" class="primary" data-next-v076="operating">\u7ee7\u7eed\u914d\u7f6e \u2192</button></div></header><div class="analysis-definition-grid-v076"><div class="analysis-definition-card-v076"><span>Project</span><b>${safe(ctl.project?.name||state.activeProjectId)}</b><small>${safe(state.activeProjectId)}</small></div><div class="analysis-definition-card-v076"><span>Solution</span><b>${safe(record?.design?.name||'-')}</b><small>${safe(record?.design?.id||'-')}</small></div><div class="analysis-definition-card-v076"><span>Motor Revision</span><b>Rev.${safe(record?.revision?.revision??'-')}</b><small>${safe(record?.revision?.id||'-')} \u00b7 ${safe(record?.revision?.content_hash||'')}</small></div><div class="analysis-definition-card-v076"><span>Analysis Revision</span><b>Rev.${safe(latest?.revision??'-')}</b><small>${safe(latest?.id||'-')} \u00b7 ${safe(latest?.content_hash||'')}</small></div><div class="analysis-definition-card-v076"><span>\u5206\u6790\u6a21\u5757</span><b>${safe(moduleLabels[ctl.active.module]||ctl.active.module)}</b><small>${safe(ctl.active.module)}</small></div><div class="analysis-definition-card-v076"><span>\u8ba1\u7b97\u914d\u65b9</span><b>${safe(recipeById(ctl.active.recipe_id)?.label||ctl.active.recipe_id)}</b><small>${safe(ctl.active.recipe_id)}</small></div></div><div class="subpanel"><div class="section-head"><div><h3>\u5207\u6362\u540c\u65b9\u6848\u7684\u7535\u673a Revision</h3><p>\u5207\u6362\u540e\u5206\u6790\u5b9a\u4e49\u4fdd\u6301\u4e0d\u53d8\uff0c\u6267\u884c\u8ba1\u5212\u4f1a\u7ed1\u5b9a\u65b0\u7684 Design Revision\u3002\u8de8\u65b9\u6848\u8bf7\u65b0\u5efa\u5206\u6790\u914d\u7f6e\u3002</p></div></div><div class="form-grid"><label>Design Revision<select id="analysisDefinitionRevisionV076">${sameDesign.map(rev=>`<option value="${safe(rev.id)}" ${String(rev.id)===String(ctl.active.design_revision_id)?'selected':''}>Rev.${safe(rev.revision)} \u00b7 ${safe(rev.id)}</option>`).join('')}</select></label><div class="actions"><button id="analysisApplyRevisionV076" type="button">\u66f4\u65b0\u5f15\u7528</button></div></div><div id="analysisDefinitionStatusV076" class="analysis-inline-status-v076"></div></div></section>`;
  ...
  async function saveOperatingCases(){const status=q('#analysisOperatingStatusV076');try{await saveAnalysisRevision({load_cases:collectCases(),notes:'Updated operating points from unified analysis configuration'});if(status){status.textContent=text.saved;status.className='analysis-inline-status-v076 success'}notify(text.saved,'SUCCESS')}catch(error){if(status){status.textContent=error.message;status.className='analysis-inline-status-v076 error'}notify(error.message,'ERROR',9000)}}
  ...
  function setHmiMode(mode){
    ctl.hmiMode=mode==='advanced'?'advanced':'common';localStorage.setItem('mcs-analysis-hmi-mode',ctl.hmiMode);localStorage.removeItem('mcs-analysis-hmi-mode-v081a');syncHmiModeButtons();if(!ctl.active)return renderCreateForm();if(['definition','inputs','solver','check'].includes(ctl.step))renderEditorBody();
  }
  ...
  function renderInputs(){
    ...
    qa('[data-domain-v076]',body).forEach(button=>button.addEventListener('click',()=>{ctl.domainId=button.dataset.domainV076;renderInputs()}));q('#analysisSaveDomainV076',body)?.addEventListener('click',saveInputDomain);q('[data-refresh-input-v076]',body)?.addEventListener('click',refreshInputs);q('[data-next-v076]',body)?.addEventListener('click',()=>setStep('solver',{replace:false}));q('[data-show-advanced-input-v081a]',body)?.addEventListener('click',()=>setHmiMode('advanced'));
  }
  async function refreshInputs(){ctl.inputCatalog=await apiCall(`/api/analysis-definitions/${encode(ctl.active.id)}/input-domains`);renderInputs();renderSummary()}
  async function saveInputDomain(){
    const domain=(ctl.inputCatalog?.domains||[]).find(row=>row.id===ctl.domainId),status=q('#analysisDomainStatusV076'),button=q('#analysisSaveDomainV076');if(!domain)return;
    const values={};qa('[data-domain-field-v076]').forEach(input=>{const field=(domain.fields||[]).find(row=>String(row.id)===String(input.dataset.domainFieldV076));if(input.tagName==='SELECT'&&input.dataset.fieldTypeV076==='boolean')values[input.dataset.domainFieldV076]=input.value==='true';else values[input.dataset.domainFieldV076]=scalarFromInput(input,field)});
    try{button.disabled=true;if(status)status.textContent='\u6b63\u5728\u4fdd\u5b58\u2026';const saved=await apiCall(`/api/analysis-definitions/${encode(ctl.active.id)}/input-domains/${encode(domain.id)}`,{method:'PUT',body:JSON.stringify({values,notes:`Updated ${domain.label||domain.id} from unified analysis configuration`})});ctl.active=saved.analysis_definition;ctl.inputCatalog=saved.catalog;ctl.definitions=ctl.definitions.map(row=>row.id===ctl.active.id?ctl.active:row);contextStore()?.setAnalysis?.(ctl.active,{analysisRevisionId:ctl.active.revisions?.[0]?.id||null,motorRevision:ctl.active.design_revision_id,source:'analysis:input-save'});ctl.executionPlan=null;ctl.fullCheck=null;await refreshGuidance();renderInputs();renderList();renderSummary();notify(text.saved,'SUCCESS')}catch(error){button.disabled=false;if(status){status.textContent=error.message;status.className='analysis-inline-status-v076 error'}notify(error.message,'ERROR',9000)}
  }
  ...
  async function submitExecution(){
    if(!ctl.active||!ctl.fullCheck?.valid)return;const status=q('#analysisCheckStatusV076'),button=q('#analysisSubmitV076');
    try{button.disabled=true;if(status)status.textContent='\u6b63\u5728\u91cd\u65b0\u51bb\u7ed3 Execution Plan \u5e76\u63d0\u4ea4\u2026';const plan=await loadExecutionPlan({render:false});if(!plan?.can_submit)throw new Error('\u6267\u884c\u8ba1\u5212\u5df2\u53d8\u66f4\u6216\u5b58\u5728\u963b\u65ad\u9879\uff0c\u8bf7\u91cd\u65b0\u68c0\u67e5\u3002');if(ctl.fullCheck.analysisRevisionId!==plan.analysis_revision?.id||ctl.fullCheck.designRevisionId!==plan.design_revision?.id)throw new Error('Revision \u5df2\u53d8\u66f4\uff0c\u8bf7\u91cd\u65b0\u8fd0\u884c\u5b8c\u6574\u8ba1\u7b97\u524d\u68c0\u67e5\u3002');const quality=q('#analysisQualityV076')?.value||ctl.qualityProfile||'standard',reuse=q('#analysisReuseV076')?q('#analysisReuseV076').checked:ctl.reuseCache,name=q('#analysisTaskNameV076')?.value.trim()||ctl.active.name;ctl.qualityProfile=quality;ctl.reuseCache=Boolean(reuse);const created=await apiCall(`/api/analysis-definitions/${encode(ctl.active.id)}/execute`,{method:'POST',body:JSON.stringify({name,quality_profile:quality,reuse_cache:reuse,submission_key:newSubmissionKey(),precheck_evidence_id:ctl.fullCheck?.result?.evidence?.id||undefined,run_native_precheck:true,expected_analysis_revision_id:plan.analysis_revision?.id,expected_design_revision_id:plan.design_revision?.id,expected_execution_plan_hash:plan.execution_plan_hash})});contextStore()?.setExecution?.(created,{taskId:created.task_id,source:'analysis:execute'});notify(created.idempotent_replay?'\u5df2\u6062\u590d\u540c\u4e00\u6b21\u8ba1\u7b97\u63d0\u4ea4':'\u8ba1\u7b97\u4efb\u52a1\u5df2\u521b\u5efa','SUCCESS',7000);const next=created.next_route||`/app/projects/${encode(state.activeProjectId)}/simulation/monitor/${encode(created.task_id)}`;return window.MCSRouter?.navigate?.(next)}catch(error){button.disabled=false;if(status){status.textContent=error.message;status.className='analysis-inline-status-v076 error'}notify(error.message,'ERROR',10000)}
  }

  async function saveAnalysisRevision(patch={}){
    if(!ctl.active)throw new Error('\u6ca1\u6709\u5f53\u524d\u5206\u6790\u914d\u7f6e');const current=activeDefinition();const payload={load_cases:patch.load_cases??current.load_cases??[{}],solver_settings:patch.solver_settings??current.solver_settings??{},input_domains:patch.input_domains??current.input_domains??{},requested_outputs:patch.requested_outputs??current.requested_outputs??[],notes:patch.notes||'Updated from unified analysis configuration'};const updated=await apiCall(`/api/analysis-definitions/${encode(ctl.active.id)}/revisions`,{method:'POST',body:JSON.stringify(payload)});ctl.active=updated;ctl.definitions=ctl.definitions.map(row=>row.id===updated.id?updated:row);ctl.inputCatalog=await apiCall(`/api/analysis-definitions/${encode(updated.id)}/input-domains`);ctl.executionPlan=null;ctl.fullCheck=null;contextStore()?.setAnalysis?.(updated,{analysisRevisionId:updated.revisions?.[0]?.id||null,motorRevision:updated.design_revision_id,source:'analysis:save-revision'});await refreshGuidance();renderList();renderSummary();return updated
  }

  function renderSummary(){
    const root=q('#analysisSummaryV076');if(!root)return;const context=currentContext(),record=activeRevisionRecord()||ctl.revisionIndex.get(String(ctl.selectedRevisionId||'')),definition=activeDefinition(),input=ctl.inputCatalog,plan=ctl.executionPlan;const cases=(definition.load_cases||[]).length,outputs=(definition.requested_outputs||[]).length,configured=(input?.domains||[]).filter(row=>row.configured).length,required=(input?.required_domain_ids||[]).length,missing=(input?.missing_required_domain_ids||[]).length;root.innerHTML=`<div class="analysis-summary-chain-v076"><div class="analysis-summary-node-v076 active"><span>Project</span><b>${safe(ctl.project?.name||context.projectId||'-')}</b></div><div class="analysis-summary-node-v076 ${record?'active':''}"><span>Solution</span><b>${safe(record?.design?.name||context.solutionId||'-')}</b></div><div class="analysis-summary-node-v076 ${record?'active':''}"><span>Motor Revision</span><b>${record?`Rev.${safe(record.revision?.revision)} \u00b7 ${safe(record.revision?.id)}`:safe(context.motorRevisionId||'-')}</b></div><div class="analysis-summary-node-v076 ${ctl.active?'active':''}"><span>Analysis</span><b>${safe(ctl.active?.name||context.analysisId||'-')}</b></div><div class="analysis-summary-node-v076 ${activeAnalysisRevision()?'active':''}"><span>Analysis Revision</span><b>${activeAnalysisRevision()?`Rev.${safe(activeAnalysisRevision().revision)} \u00b7 ${safe(activeAnalysisRevision().id)}`:'-'}</b></div><div class="analysis-summary-node-v076 ${context.taskId?'active':''}"><span>Execution</span><b>${safe(context.taskId||'\u5c1a\u672a\u63d0\u4ea4')}</b></div></div><div class="analysis-readiness-v076"><div><span>\u5de5\u51b5</span><b>${cases}</b></div><div><span>\u8bf7\u6c42\u8f93\u51fa</span><b>${outputs}</b></div><div><span>\u7269\u7406\u8f93\u5165</span><b>${configured}${required?`/${required}`:''}</b></div><div><span>\u5fc5\u586b\u7f3a\u5931</span><b>${missing}</b></div></div><div class="analysis-summary-note-v076">${plan?`Execution Plan ${safe(String(plan.execution_plan_hash||'').slice(0,12))}\u2026 \u00b7 ${plan.can_submit?'\u5de5\u7a0b\u5408\u540c\u5c31\u7eea':'\u5b58\u5728\u5f85\u5904\u7406\u9879'}`:'\u5206\u6790\u914d\u7f6e\u9875\u4e0d\u518d\u53e6\u5916\u7ef4\u62a4\u4efb\u52a1\u5411\u5bfc\u72b6\u6001\u3002\u5f53\u524d\u5bf9\u8c61\u94fe\u7531 Engineering Context Store \u7edf\u4e00\u7ba1\u7406\u3002'}</div>`;
  ...
  async function refresh(){
    if(!state.activeProjectId)return;const activeId=ctl.active?.id||ctl.activeId;await fetchProjectGraph();await Promise.all([fetchDefinitions(),fetchCatalogForRevision(),fetchTemplateCatalogForRevision()]);if(activeId&&ctl.definitions.some(row=>row.id===activeId))await hydrateActive(activeId);else{ctl.active=null;ctl.activeId=null;ctl.inputCatalog=null;renderAll()}
  }
  ...
  async function mount(route,ctx){
    ctl.ctx=ctx||null;ctl.route=route||{};ctl.step=resolveStep(route);ctl.executionPlan=null;ctl.fullCheck=null;contextStore()?.setStage?.('analysis',{source:'analysis:mount'});
    ...
    catch(error){if(window.MCSPageRuntime?.isAbortError?.(error))return;const body=q('#analysisEditorBodyV076');showEditor();if(body)body.innerHTML=`<div class="analysis-empty-v076"><h3>${safe(text.loadFailed)}</h3><p>${safe(error.message)}</p><button type="button" data-analysis-retry-v076>\u91cd\u8bd5</button></div>`;q('[data-analysis-retry-v076]',body)?.addEventListener('click',()=>refresh());throw error}
  }

  function unmount(){ctl.requestToken+=1;ctl.ctx=null;ctl.route=null;ctl.busy=false}

  window.MCSUnifiedAnalysis={mount,unmount,refresh,openCreate:renderCreateForm,setStep,state:ctl};
})();
