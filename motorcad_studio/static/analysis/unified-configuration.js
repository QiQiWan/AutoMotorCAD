...
(() => {
  ...
  const ctl={
    ...
    submissionKey:null,submissionFingerprint:null,transitionBusy:false,loadPromise:null,definitionsProjectId:null,workspaceClient:null,guidanceRequestToken:0,guidanceTimer:null,
  ...
  function refresh(options={}){
    if(ctl.loadPromise)return ctl.loadPromise;
    const promise=refreshImpl(options);ctl.loadPromise=promise;
    return promise.finally(()=>{if(ctl.loadPromise===promise)ctl.loadPromise=null});
  }
  ...
  function mountSingleFlight(route,ctx){
    if(ctl.loadPromise)return ctl.loadPromise;
    const promise=mount(route,ctx);ctl.loadPromise=promise;
    return promise.finally(()=>{if(ctl.loadPromise===promise)ctl.loadPromise=null});
  }
