/* Stable routed page-controller + idempotent submission owner (migrated from V0.25 in V0.66). */
(() => {
  ...
  async function mountResults(route, ctx) {
    await window.MCSResultsWorkbenchV069?.mount?.(route,ctx);
    ctx.assertActive();
  }

  async function mountAnalysis(route, ctx) {
    await window.MCSV040?.mountAnalysisWorkbench?.(ctx); ctx.assertActive();
    if (!route.analysisId) return;
    if (window.MCSV060?.state) window.MCSV060.state.activeId=route.analysisId;
    await window.MCSV060?.renderAnalysisCaseWorkbench?.(); ctx.assertActive();
    if (route.analysisAction === 'execute' && window.MCSAnalysisExecution?.open) {
      ctx.onDispose(()=>window.MCSAnalysisExecution?.close?.());
      await window.MCSAnalysisExecution.open(route.analysisId, false, ctx, route.analysisStep||null);
      ctx.assertActive();
    }
  }
  ...
  async function mount(route, ctx) {
    ...
    switch (route.tab) {
      ...
      case 'analysisWorkbench': await mountAnalysis(route,ctx); break;
      ...
      case 'resultViewer': await mountResults(route,ctx); break;
