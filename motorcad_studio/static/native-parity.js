/* V0.73-A Native Closure. Current UI uses version-neutral closure DOM names. */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const statusLabel=(s)=>({PASS:'已通过',FAIL:'未通过',NOT_RUN:'未执行',STALE:'已过期',BINDING_ERROR:'绑定错误',WARN:'需关注',QUALIFIED:'已固化',PARTIAL:'部分解析',UNRESOLVED:'未解析',MISSING:'待采集'}[s]||s||'未执行');
  ...
  function renderProfiles(){
    ...
    box.innerHTML=catalog.map(p=>{const latest=p.latest||{},score=latest.score||{},semantic=semanticProfile(p.template_id);return `<article class="native-parity-profile-closure ${latest.qualified?'qualified':''}">
      ...
      <div class="native-parity-profile-meta-closure"><span>模板 <b>${esc(p.template_id||'-')}</b></span><span>Motor-CAD <b>${esc(p.target_motorcad_version||'-')}</b></span><span>PyMotorCAD <b>${esc(p.required_pymotorcad_version||'-')}</b></span><span>Native Binding <b>${esc(bindingCatalog.binding_version||'-')}</b></span><span>语义绑定 <b>${esc(statusLabel(semantic.status))}</b></span><span>Scope <b>${esc(String(latest.qualification_key||'-').slice(0,12))}</b></span><span>得分 <b>${score.percent??0}%</b></span></div>
  ...
  function renderMatrix(matrix){
    ...
    const semText=`V0.88-A 精确语义绑定 ${sem.qualified_count||0}/${sem.golden_template_count||3} 个 Golden 模板已固化。`;
    q('#nativeParitySummaryClosure').textContent=(matrix?.complete?'BPM / SPM / IPM / AFPM 已全部在当前工作站通过原生一致性门禁。':`已通过 ${matrix?.qualified_profiles||0}/${matrix?.total_profiles||rows.length} 个 Profile；未通过项不会标记 NATIVE_QUALIFIED。`)+` ${semText}`;
    box.innerHTML=`<div class="table-wrap native-parity-table-closure"><table><thead><tr><th>Profile</th><th>模板</th><th>状态</th><th>得分</th><th>最近证据</th><th>阻断项</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${esc(r.label||r.profile_id)}</b><small>${esc(r.profile_id)}</small></td><td>${esc(r.template_id||'-')}</td><td><span class="native-parity-state-closure ${statusClass(r.status)}">${esc(statusLabel(r.status))}</span></td><td>${esc((r.score||{}).percent??0)}%</td><td>${r.run_id?`<button type="button" class="link-button" data-native-parity-detail="${esc(r.run_id)}">${esc(r.run_id)}</button>`:'-'}</td><td>${esc((r.blocking_checks||[]).join(', ')||'—')}</td></tr>`).join('')}</tbody></table></div>`;
  }
