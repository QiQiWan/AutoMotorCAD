...
DEFAULT_SECTION_PREFERENCES: dict[str, list[str]] = {
    ...
    "RotorDiameter": ["Dimensions"],
    "RotorOuterDiameter": ["Dimensions"],
    "AFM_D_Rotor": ["Dimensions"],
    "Magnet_Width": ["Dimensions"],
    "Magnet_Embed_Depth": ["Dimensions"],
    "Pole_V_Angle": ["Dimensions"],
    "Magnet_Separation": ["Dimensions"],
    "VMagnet_Layers": ["Dimensions"],
...
DEFAULT_PARAMETER_SOURCES: dict[str, str] = {
    ...
    "rotor_outer_diameter": "RotorOuterDiameter",
    "axial_rotor_diameter": "AFM_D_Rotor",
    "magnet_width": "Magnet_Width",
    "magnet_embed_depth": "Magnet_Embed_Depth",
    "pole_v_angle_deg": "Pole_V_Angle",
    "magnet_separation": "Magnet_Separation",
    "magnet_layers": "VMagnet_Layers",
...
def extract_parameter_sources_with_metadata(
    path: Path, sources: dict[str, dict[str, Any]] | None
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Extract plugin-contributed canonical defaults using explicit MTT source keys."""
    sections = parse_mtt(path)
    defaults: dict[str, Any] = {}
    metadata: dict[str, Any] = {}
    for canonical, source in dict(sources or {}).items():
        spec = dict(source or {})
        key = str(spec.get("key") or "")
        if not key:
            continue
        raw, info = resolve_value(sections, key, spec.get("sections"))
        value = coerce_number(raw)
        info.update({"canonical": canonical, "raw_value": raw, "value": value, "plugin_source": True})
        metadata[canonical] = info
        if value is not None:
            defaults[canonical] = value
    return defaults, metadata
...
def extract_material_sources(path: Path, sources: dict[str, list[str]] | None) -> tuple[dict[str, str], dict[str, Any]]:
    """Extract plugin-owned component material assignments from the MTT Material section."""
    sections = parse_mtt(path)
    material = sections.get("Material") or {}
    defaults: dict[str, str] = {}
    metadata: dict[str, Any] = {}
    for component, keys in dict(sources or {}).items():
        candidates = [str(key) for key in keys]
        selected_key = next((key for key in candidates if str(material.get(key, "")).strip()), None)
        value = str(material.get(selected_key, "")).strip() if selected_key else ""
        metadata[str(component)] = {
            "component": str(component), "candidate_keys": candidates, "selected_key": selected_key,
            "source_section": "Material", "source": "motor_family_plugin",
        }
        if value:
            defaults[str(component)] = value
    return defaults, metadata
