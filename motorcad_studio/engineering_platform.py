...
class EngineeringPlatformService:
    ...
    def _recipe_capability(self, recipe_id: str, spec: dict[str, Any], motor_type_id: str, template_id: str | None = None) -> dict[str, Any]:
        ...
        qualification_current = int(qualification_payload.get("qualification_contract_version") or 0) >= 3
    ...
    def analysis_catalog(self, motor_type_id: str | None = None, template_id: str | None = None) -> dict[str, Any]:
        motor_type = str(motor_type_id or self.default_motor_type)
        modules = set((self.motor_types.get(motor_type) or {}).get("modules") or [])
        recipes = []
        for recipe_id, spec in self.registry.analysis_recipe_schema().items():
    ...
    def capability_snapshot(self, motor_type_id: str) -> dict[str, Any]:
        catalog = self.analysis_catalog(motor_type_id)
        contexts = {context for row in catalog["recipes"] if row["available"] for context in row["contexts"]}
        automation = {}
        for context in sorted(contexts):
            payload = self.automation.get(AutomationRegistryKey(self.registry.motorcad_version, motor_type_id, context))
            automation[context] = {
    ...
    def create_model(self, project_id: str, request: ModelCreate) -> dict[str, Any]:
        ...
        if kind == ModelSourceKind.REVISION_CLONE:
            ...
            motor_type = str(source_design.get("motor_type_id") or source_design.get("motor_family") or motor_type)
    ...
    def parameter_catalog(self, revision_id: str, context: str | None = None) -> dict[str, Any]:
        ...
        motor_type = str(design.get("motor_type_id") or design.get("motor_family") or self.default_motor_type)
    ...
    def _normalize_analysis_definition(self, recipe_id: str, load_cases: list[dict[str, Any]], solver_settings: dict[str, Any], requested_outputs: list[str], input_domains: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
        spec = self.registry.analysis_recipe_schema().get(recipe_id)
        if not spec:
        ...
        output_schema = self.registry.output_schema()
    ...
    def create_analysis_definition(self, project_id: str, request: AnalysisDefinitionCreate) -> dict[str, Any]:
        ...
        design = self.db.query_one("SELECT project_id,motor_type_id,motor_family FROM designs WHERE id=?", (revision["design_id"],)) or {}
        if design.get("project_id") != project_id:
            raise ValueError("Design Revision 不属于当前项目")
        catalog = {row["id"]: row for row in self.analysis_catalog(str(design.get("motor_type_id") or design.get("motor_family")))["recipes"]}
        ...
        now = self.db.now()
        definition = self._normalize_analysis_definition(request.recipe_id.value, request.load_cases, request.solver_settings, request.requested_outputs, request.input_domains)
        ...
        with self.db.transaction() as conn:
            ...
            conn.execute(
                "INSERT INTO analysis_definition_revisions(id,analysis_definition_id,revision,definition_json,notes,content_hash,created_at) VALUES(?,?,?,?,?,?,?)",
                (rid, aid, 1, self.db.dumps(definition), request.notes, _hash(definition), now),
            )
    ...
    def create_analysis_revision(self, analysis_id: str, request: AnalysisDefinitionRevisionCreate) -> dict[str, Any]:
        ...
        if not parent:
            raise KeyError(analysis_id)
        recipe = self.registry.analysis_recipe_schema().get(parent["recipe_id"], {})
        ...
        if not input_domains:
            ...
            input_domains = self.db.loads(latest.get("definition_json"), {}).get("input_domains") or {}
        definition = self._normalize_analysis_definition(str(parent["recipe_id"]), request.load_cases, request.solver_settings, request.requested_outputs, input_domains)
        ...
        with self.db.transaction() as conn:
            ...
            conn.execute(
                "INSERT INTO analysis_definition_revisions(id,analysis_definition_id,revision,definition_json,notes,content_hash,created_at) VALUES(?,?,?,?,?,?,?)",
                (rid, analysis_id, number, self.db.dumps(definition), request.notes, _hash(definition), now),
            )
