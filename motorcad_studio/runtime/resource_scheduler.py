...
class RuntimeResourceScheduler:
    ...
    def startup(self) -> dict[str, Any]:
        """Re-open admission after an application lifespan restart."""
        with self._condition:
            ...
            return {
                "authority": "RuntimeSchedulerLifecycleV1",
                "state": "OPEN",
    ...
    def shutdown(self, *, wait_timeout_s: float = 5.0) -> dict[str, Any]:
        """Close admission, wake queued waiters, then wait briefly for active leases."""
        started = time.monotonic()
        deadline = started + max(0.0, float(wait_timeout_s))
        with self._condition:
            self._closed = True
            self._closed_at = _utc_now()
            cancelled_queue = len(self._queue)
            self._cancellations += cancelled_queue
            self._queue.clear()
            self._condition.notify_all()
            while self._active and time.monotonic() < deadline:
                self._condition.wait(timeout=min(self.wait_poll_s, max(0.01, deadline - time.monotonic())))
            residual = [row.to_dict() for row in self._active.values()]
            evidence = {
                "authority": "RuntimeSchedulerShutdownV1",
                "state": "CLOSED",
                "generation": self._lifecycle_generation,
                "closed_at": self._closed_at,
                "cancelled_queued_requests": cancelled_queue,
                "residual_active_leases": residual,
                "clean": not residual,
    ...
    def acquire(
        ...
        *,
        ...
        with self._condition:
            if self._closed:
                raise RuntimeResourceCancelled("Motor-CAD运行时资源调度器正在关闭，拒绝新的资源请求")
            ...
            try:
                while True:
                    if self._closed:
                        ...
                        raise RuntimeResourceCancelled("Motor-CAD运行时资源调度器已关闭")
    ...
    def readiness(self) -> dict[str, Any]:
        ...
        if self._closed:
            issues.append({"severity": "BLOCKING", "code": "RUNTIME_SCHEDULER_CLOSED", "message": "运行时资源调度器当前已关闭。"})
