...
def _child(payload: dict[str, Any], result_queue: Any) -> None:
    try:
        ...
        result = adapter.qualify_native_closure(
...
class MotorCADNativeClosureRunner:
    """Run one V0.73-A Native Closure profile in a killable child process.
            ...
            while True:
                ...
                if response is not None:
                    ...
                    return {
                        ...
                        "qualified": False,
                        "status": "FAIL",
                        "checks": [{"id": "native_closure_worker", "domain": "runtime", "required": True, "status": "FAIL", "message": response.get("error") or "native closure worker failed"}],
                ...
                if time.monotonic() - started >= self.timeout_s:
                    ...
                    return {
                        ...
                        "qualified": False,
                        "status": "FAIL",
                        "checks": [{"id": "native_closure_timeout", "domain": "runtime", "required": True, "status": "FAIL", "message": f"Native Closure 超过 {self.timeout_s:.0f}s，已终止 Motor-CAD 子进程树"}],
                    }
                if not process.is_alive():
                    try:
                        ...
                        return {
                            ...
                            "qualified": False,
                            "status": "FAIL",
                            "checks": [{"id": "native_closure_worker", "domain": "runtime", "required": True, "status": "FAIL", "message": f"Native Closure 子进程异常退出，exitcode={process.exitcode}"}],
                        }
