...
def _child_main(payload: dict[str, Any], event_conn: Any) -> None:
    ...
    def runtime_log(level: str, event_type: str, message: str, *, stage: str | None = None, extra: dict[str, Any] | None = None) -> None:
        record = {
            ...
            "stage": stage,
            "trace_id": payload.get("execution_plan_hash") or payload.get("task_id"),
            "run_id": payload.get("execution_plan_id") or payload.get("case_id"),
            "topology_id": ((payload.get("motor_snapshot") or {}).get("identity") or {}).get("topology_id"),
    ...
    try:
        ...
        # V0.75-B: load Motor Family plugins before workstation result calibration.
        # Plugin-contributed outputs (for example IM rotor copper loss / power factor)
        # must be registered before calibration entries are filtered by result_id.
        plugin_registry = create_motor_plugin_registry(
            registry, Path(payload["config_dir"]), studio_version=str(payload.get("studio_version") or __version__)
        )
        registry.attach_motor_plugins(plugin_registry)
