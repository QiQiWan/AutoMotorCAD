...
def reconcile_inherited_runtime_baseline(
    ...
    """Align untouched template values to the model that Motor-CAD actually loaded.

    A source checkout can contain an MTT captured from an earlier 2026R1 build while
    ``load_template()`` opens the registered template from the engineer's installed
    2026R1 build. Explicit Design values remain Studio authority; untouched values
    inherit the live registered-template baseline until a verified local MOT exists.
    """
...
class MotorCADRuntimeAdapter(MotorCADSolverAdapter):
    ...
    Deterministic binding remains in ``native/motorcad``. This facade owns only live
    workstation concerns: registered-template baseline alignment and final geometry
    recovery state convergence.
    """
    ...
    def qualify_template(
        ...
        **kwargs: Any,
        ...
        """Use the live registered template as baseline for untouched parameters.

        This applies only to development fallback when no verified MOT exists. User
        explicit parameters are never replaced and remain fail-closed write/readback
        intent.
        """
