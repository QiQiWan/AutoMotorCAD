...
from ..native_parity import compare_values, classify_parameter_tolerance, summarize_check, finalize_parity_result, evidence_hash
...
class MotorCADSolverAdapter(SolverAdapter):
    def __init__(
        ...
        self.model_policy = model_policy if model_policy in {"development", "validation", "production", "native_parity"} else "development"
    ...
    def _load_model(self, mc: Any, template: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "verified": False,
            "policy": self.model_policy,
            "candidate_baseline": self.model_policy == "native_parity",
            "warning": (
                "未找到本地验收MOT母版；V0.68 将以目标 2026R1 注册模板建立候选基线，只有完整 Native Parity 通过后才固化本地 MOT。"
                if self.model_policy == "native_parity" else
    ...
    def _write_native_parity_report(result: dict[str, Any], path: Path) -> None:
        ...
        lines = [
            f"# Motor-CAD Native Parity — {result.get('profile_id','')}", "",
    ...
    def qualify_native_parity(
        ...
        *,
        ...
        """Run a target-workstation parity contract against one native Motor-CAD model.
        ...
        try:
            ...
            checks.append({
                ...
                "message": (
                    f"PyMotorCAD {actual_pymotorcad_version} 与 V0.68 固化资格版本一致"
            ...
            # V0.68 parity validates both directions of the Studio automation contract.
            # Reading native defaults alone can hide a stale write mapping, therefore we
            # idempotently write the same values back through Studio's canonical mapper
            # and require a second native readback to match. This is safe for a baseline
            # model because no design value is intentionally perturbed.
            expected_parameters = {
            ...
            for context in ("EMag", "Therm"):
                try:
                    warnings_part, audit_part = self._apply_parameters(
                        mc, template["id"], expected_parameters, lambda *_: None,
                        context=context, progress_start=0.0, progress_end=1.0,
                    )
                    roundtrip_warnings.extend(warnings_part)
                    roundtrip_audit.update(audit_part)
                except Exception as exc:
                    roundtrip_errors.append(f"{context}: {type(exc).__name__}: {exc}")
            try:
                dependency_audit, dependency_warnings = self._apply_template_dependencies(
                    mc, template["id"], expected_parameters, set(expected_parameters)
                )
                roundtrip_audit.update(dependency_audit)
                roundtrip_warnings.extend(dependency_warnings)
            except Exception as exc:
                roundtrip_errors.append(f"dependencies: {type(exc).__name__}: {exc}")
            post_write_defaults = self._runtime_defaults(mc, template["id"], all_parameter_ids)
            roundtrip_rows: list[dict[str, Any]] = []
            for parameter_id in all_parameter_ids:
                expected = expected_parameters.get(parameter_id)
                native = post_write_defaults.get(parameter_id) or {}
                actual = native.get("value")
                definition = schema.get(parameter_id) or {}
                tolerance = classify_parameter_tolerance(parameter_id, definition, tolerances)
                comparison = compare_values(expected, actual, **tolerance) if expected is not None and actual is not None else {"matched": False, "expected": expected, "actual": actual}
                write_row = roundtrip_audit.get(parameter_id) or {}
                write_mapping_ok = bool(write_row.get("motorcad_variable")) and write_row.get("matched") is not False
                matched = bool(comparison.get("matched")) and write_mapping_ok
                roundtrip_rows.append({
                    "parameter_id": parameter_id,
                    "motorcad_variable": write_row.get("motorcad_variable") or native.get("source"),
                    "write_mapping_ok": write_mapping_ok,
                    "write_readback": write_row.get("readback"),
                    "native_readback": actual,
                    "status": "PASS" if matched else "FAIL",
                    **{**comparison, "matched": matched},
                })
            if roundtrip_errors:
                roundtrip_rows.append({"parameter_id": "__roundtrip_runtime__", "status": "FAIL", "matched": False, "errors": roundtrip_errors})
            checks.append(summarize_check(
                "parameter_write_roundtrip", "automation", roundtrip_rows, required=True,
                message="Studio canonical 参数写入 → Motor-CAD 原生回读逐项闭环",
            ))
            result["parameter_write_roundtrip"] = {"rows": roundtrip_rows, "audit": roundtrip_audit, "warnings": roundtrip_warnings, "errors": roundtrip_errors}
            ...
            winding_snapshot = self._native_winding_snapshot(mc, template, runtime_defaults)
            ...
            requested_materials = {
                component: expected_materials.get(component)
                for component in profile.get("required_material_components") or []
                if expected_materials.get(component)
            }
            material_write_audit: dict[str, Any] = {}
            material_write_warnings: list[str] = []
            material_write_error: str | None = None
            try:
                material_write_audit, material_write_warnings = self._apply_materials(
                    mc, {"component_materials": requested_materials}
                )
            except Exception as exc:
                material_write_error = f"{type(exc).__name__}: {exc}"
            material_write_rows = []
            for component in profile.get("required_material_components") or []:
                row = material_write_audit.get(f"component:{component}") or {}
                expected = expected_materials.get(component)
                successes = list(row.get("successes") or [])
                matched = bool(expected) and bool(successes) and all(str(item.get("readback") or "").casefold() == str(expected).casefold() for item in successes)
                material_write_rows.append({"component": component, "expected": expected, "targets": row.get("resolved_targets") or [], "successes": successes, "errors": row.get("errors") or [], "matched": matched, "status": "PASS" if matched else "FAIL"})
            if material_write_error:
                material_write_rows.append({"component": "__material_write_runtime__", "matched": False, "status": "FAIL", "error": material_write_error})
            checks.append(summarize_check(
                "material_write_roundtrip", "materials", material_write_rows, required=True,
                message="Studio 部件材料设置 → Motor-CAD get_component_material 回读闭环",
            ))
            result["material_write_roundtrip"] = {"rows": material_write_rows, "audit": material_write_audit, "warnings": material_write_warnings, "error": material_write_error}
            for component in profile.get("required_material_components") or []:
                expected = expected_materials.get(component)
                targets = self._resolve_material_components(mc, component)
                ...
                for target in targets:
                    try:
                        values.append({"component": target, "material": mc.get_component_material(target)})
                    except Exception as exc:
                        values.append({"component": target, "error": f"{type(exc).__name__}: {exc}"})
                actual_values = [str(row.get("material") or "") for row in values if row.get("material")]
                matched = bool(expected) and bool(actual_values) and all(str(expected).casefold() == value.casefold() for value in actual_values)
                material_rows.append({"component": component, "expected": expected, "actual": actual_values, "targets": values, "matched": matched, "status": "PASS" if matched else "FAIL"})
            checks.append(summarize_check("materials", "materials", material_rows, required=True, message="Studio 模板材料与 Motor-CAD 当前组件材料逐项对照"))
            ...
            licence = self._ensure_license(mc, "EMag")
            mc.do_magnetic_calculation()
            checks.append({"id": "native_emag_solve", "domain": "solver", "required": True, "status": "PASS", "message": "真实 Motor-CAD EMag 求解完成", "licence": licence})

            required_results = list(profile.get("required_results") or [])
            scalar_ids = [result_id for result_id in required_results if (self.registry.output_schema(template["id"]).get(result_id) or {}).get("type") == "scalar"]
            series_ids = [result_id for result_id in required_results if (self.registry.output_schema(template["id"]).get(result_id) or {}).get("type") == "series"]
            scalars, scalar_audit, scalar_warnings = self._extract_scalar_outputs(mc, template["id"], scalar_ids, context="EMag")
            series, series_audit, series_warnings = self._extract_series_outputs(mc, template["id"], series_ids, context="EMag")
            result["studio_result_snapshot"] = {"scalars": scalars, "series": series, "scalar_audit": scalar_audit, "series_audit": series_audit, "warnings": scalar_warnings + series_warnings}
            output_schema = self.registry.output_schema(template["id"])
            ...
            # V0.68 is the bridge that turns a target-version registered template into
            ...
            checks.append({"id": "native_parity_exception", "domain": "runtime", "required": True, "status": "FAIL", "message": f"{type(exc).__name__}: {exc}"})
        ...
        finalize_parity_result(result)
        result["evidence_sha256"] = evidence_hash(result)
        evidence_path = work_dir / "native_parity_evidence.json"
        ...
        report_path = work_dir / "native_parity_report.md"
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            progress("PHYSICAL_INPUTS", 0.16, "应用已确认的材料、冷却、损耗和热边界输入")
