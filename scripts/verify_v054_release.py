...
def main() -> None:
    ...
    assert metrics["fea_normalization_schema"] == 5
    assert metrics["fea_frame_schema"] == 3
    assert metrics["result_extraction_contract_schema"] == 3
    ...
    table_javascript = (ROOT / "motorcad_studio/static/v054.js").read_text(encoding="utf-8")
    assert "/static/v052.js?v=0.56.0" in index
    assert "/static/v054.js?v=0.56.0" in index
    for token in ("真实三角单元填色", "抽样完整性", "data_profile"):
        assert token in javascript
    for token in ("MOTOR-CAD NATIVE TABLE", "source_sha256", "native-tables"):
        assert token in table_javascript
